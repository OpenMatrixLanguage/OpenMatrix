...
#ifndef OS_WIN // This is a temp fix as std::regex_search does not work with gcc 4.8
    if (!foundanymatch)
    {
        std::vector<Currency> tmpinputs;
        tmpinputs.push_back(search);
        tmpinputs.push_back(pattern);
     
        oml_strfind(eval, tmpinputs, outputs);
        return outputs;
    }
#endif
