	...
	, cached_line (0)
{
	...
	hidden_paths            = new std::vector<std::string>;
	restore_paths			= new std::vector<std::string>;		
	restore_hidden_paths	= new std::vector<std::string>;		
	treat_as_builtin_list   = new std::vector<std::string>;
	...
	_library_aliases        = new std::map<std::string, std::string>;
	_decryptors             = new std::map<std::string, ENCRPTR>;
	cached_path_contents    = new std::map<const std::string*, std::vector<std::string>>;
...
ExprTreeEvaluator::~ExprTreeEvaluator()
{
	...
	if (_owns_pathnames)
	{
		delete paths;
		delete hidden_paths;
		delete restore_paths;			// VSM-5385
		delete restore_hidden_paths;	// VSM-5385
		delete treat_as_builtin_list;
		delete cached_path_contents;
	}
...
// Copy constructor
//------------------------------------------------------------------------------
ExprTreeEvaluator::ExprTreeEvaluator(const ExprTreeEvaluator* source) 
	: format(source->format)
	, _owns_format(false)
	, assignment_nargout(source->assignment_nargout)
	, is_for_evalin(source->is_for_evalin)
    , _quit (false), ext_var_ptr(NULL)
    , current_tree (NULL)
    , _signalHandler (NULL)
	, end_context_varname(NULL)
	...
	, _profiling(false) // only profile one Evaluator at a time
	, _verbose(0)
	, _suppress_all(false)
	, _num_threads(1)
	, _dll_user_hierarchy(NULL)
	...
	, current_statement_index (0)
{
	...
	_decryptors = source->_decryptors; // shallow copy

	_library_aliases = source->_library_aliases; // shallow copy

	_parfor_eval = source->_parfor_eval; // not sure this is really necessary
}
...
Currency ExprTreeEvaluator::HexNumber(OMLTree* tree)
{
	if (!tree->u)
	{
		...
		tree->u = new Currency((double)val);
	}
...
Currency ExprTreeEvaluator::TextString(OMLTree* tree)
{
	if (!tree->u)
	{
		...
		if (child)
			tree->u = new Currency(std::string(child->GetText())); // cppcheck-suppress cstyleCast 
		else
			tree->u = new Currency("");
	}

	const Currency* pCur = (Currency*)tree->u; // cppcheck-suppress cstyleCast 
	return *pCur;
}
...
Currency ExprTreeEvaluator::Identifier(OMLTree* tree)
{
	...
	const std::string* pString = (std::string*)tree->u; // cppcheck-suppress cstyleCast 
...
Currency ExprTreeEvaluator::CallFunction(const std::string* func_name, const std::vector<Currency>& params)
{
	const MemoryScope* scope = msm->GetCurrentScope();
	...
	if (current_filename_ptr)
		current_filename = *current_filename_ptr;
	...
	if (hidden_local_fi)
	{
		...
		{
			FunctionInfo* local_fi1 = (*hidden_local_fi->local_functions)[func_name];
			if (local_fi1)
				return CallInternalFunction(local_fi1, params);
		}
	}

	// If this call occurs within a class method, need to check the constructor of that class for any potential local functions
	FunctionInfo* cfi = scope->GetFunctionInfo();
	if (cfi)
	{
		ClassInfo* parent_ci = cfi->GetParentClass();

		if (parent_ci)
		{
			std::string parent_class_name = parent_ci->GetClassname();
		
			if (functions->find(parent_class_name) != functions->end())
			{
				FunctionInfo* constructor_fi = (*functions)[parent_class_name]->fi;

				if (constructor_fi->local_functions->find(func_name) != constructor_fi->local_functions->end())
				{
					FunctionInfo* local_fi = (*constructor_fi->local_functions)[func_name];
					if (local_fi)
						return CallInternalFunction(local_fi, params);
				}
			}
	...
	if (params.size() && HasOverloadedFunction(params[0], *func_name))
	{
		...
		{
			std::string error_message = "Unknown class method: " + *func_name;
	...
	{
		const ClassInfo* ci = (*class_info_map)[*func_name];
	...
	else if (FindFunctionInPath(*func_name, found_file, found_ext))
	{
		if (found_file != current_filename)
		{
			...
			const UserFunc* uf = (*functions)[*func_name];
		...
		{
			std::string error_message = "Illegal recursive call detected with script: " + *func_name;
			throw OML_Error(error_message);
		}
	}
...
FunctionInfo* ExprTreeEvaluator::GetBaseClassFunctionInfo(const std::string* func_name, std::string* base_name, Currency* base_val)
{
	...
	if (pos != std::string::npos)
	{
		...
		{
			const ClassInfo* ci = (*class_info_map)[class_name];
...
{
	// Check here for overloaded functions if any of the param is an object
	...
	if (params.size())
	{
		...
		{
			if (params[j].IsObject())
			{
				if (HasOverloadedFunction(params[j], func_name))
	...
	if (_parfor_eval)
	{
		...
		if (!bif.IsThreadSafe())
			throw OML_Error("Unsupported operation: calling non thread-safe function in parfor loop");
	}
...
Currency ExprTreeEvaluator::CallBuiltinFunction(ALT_FUNCPTR alt_fptr, const std::string& func_name, const std::vector<Currency>& params)
{
	...
	EvaluatorInterface intf(this);

	OMLCurrencyListImpl* in_list  = new OMLCurrencyListImpl(&intf);
	OMLCurrencyListImpl* out_list = new OMLCurrencyListImpl(&intf);

	for (int j=0; j<params.size(); j++)
	{
		if (params[j].IsLogical())
			in_list->AddLogical(params[j].Scalar() ? true : false);
		else if (params[j].IsScalar())
	...
	OMLInterfaceImpl* impl = new OMLInterfaceImpl(&intf);
	...
	if (out_list->Size() > 0)
	{
		const OMLCurrencyImpl* temp = (OMLCurrencyImpl*)out_list->Get(0); // cppcheck-suppress cstyleCast
...
Currency ExprTreeEvaluator::VariableIndex(const Currency& target, const std::vector<Currency>& in_params)
{
	...
	if (needs_adjust)
	{
		for (int j = 0; j < (int)params.size(); ++j)
		{
			...
			if (cur.IsObject())
			{
				if (HasOverloadedFunction(cur, "subsindex"))
				{
					std::vector<Currency> input_vec;
					input_vec.push_back(cur);
					Currency temp = CallOverloadedFunction("subsindex", input_vec);
					params[j] = temp;
				}
			}
		}
	...
	if (target.IsMatrix() && (params.size() == 1))
	{
		...
		else if (p0.IsPositiveIntegralVector())
		{
			if (p0.GetMask() == Currency::MASK_DOUBLE)
			{
				...
				{
					if (data->M() == 1)
						ret = allocateMatrix(1, mtx->Size(), data->IsReal());
					else
						ret = allocateMatrix(mtx->Size(), 1, data->IsReal());
				}
				else
				{
					ret = allocateMatrix(mtx->M(), mtx->N(), data->IsReal());
				}
		...
		else if (p0.IsVector() && p0.IsLogical())
		{
			...
			if (data->IsReal())
			{
				...
				hwMatrix* ret_mat = allocateMatrix(new_rows, new_cols, true);
			...
			else
			{
				...
				hwMatrix* ret_mat = allocateMatrix((int)ret.size(), 1, true);
		...
		else if (p0.IsMatrix())
		{
			...
			else if (p0.IsLogical())
			{
				if ((data->M() == val->M()) && (data->N() == val->N()))
				{
					if (data->IsReal())
					{
						...
						hwMatrix* temp = allocateMatrix((int)temp_vec.size(), 1, true);
					...
					else
					{
						...
						hwMatrix* temp = allocateMatrix((int)temp_vec.size(), 1, false);
			...
			else
			{
				...
				if (data)
				{
					ret = allocateMatrix(indices->M(), indices->N(), data->IsReal());
		...
		else if (p0.IsColon())
		{
			...
			if (data)
			{
				ret = allocateMatrix(data->Size(), 1, data->IsReal());
	...
	else if ((target.IsMatrixOrString() && (params.size() == 2)))
	{
		...
		else if (p0.IsScalar() && p1.IsColon())  // get the ith row
		{
			...
			hwMatrix* ret = allocateMatrix(1, data->N(), data->IsReal());
		...
		else if (p0.IsColon() && p1.IsScalar())  // get the ith column
		{
			...
			hwMatrix* ret = allocateMatrix(data->M(), 1, data->IsReal());
		...
		else if (p0.IsVector() || p1.IsVector())
		{
			...
			hwMatrix* ret = allocateMatrix((int)first_range.size(), (int)second_range.size(), data->IsReal());
		...
		else if (p0.IsEmpty() || p1.IsEmpty())
		{
			...
			hwMatrix* ret = allocateMatrix(new_rows, new_cols, data->IsReal());
	...
	else if (target.IsNDMatrix() || (target.IsMatrix() && params.size() > 2))
	{
		...
		if (params.size() == 1)
		{
			...
			else if (p0.IsColon())
			{
				...
				mat_n->SliceRHS(slices1, temp_slice);
			...
			else if (p0.IsMatrix())
			{
    			...
    			hwMatrix* retm = allocateMatrix(idxm->M(), idxm->N(), mat_n->IsReal());
	...
	{	
		...
		for (int j = 0; j < params.size(); j++)
		{
			...
			{
				slices1.push_back((int)temp.Scalar() - 1);
			}
			else if (temp.IsPositiveIntegralMatrix())
			{
				if (temp.IsPositiveIntegralVector() && (params.size() != 1))
				{
					...
					slices1.push_back(hwSliceArg(temp_int));
		...
		if (all_scalars)
		{
			...
			{
				hwSliceArg arg1 = slices1[0];
				int idx1 = arg1.Scalar();
				hwSliceArg arg2 = slices1[1];
		...
		else
		{
			...
			mtxs->SliceRHS(slices1, *temp_slice);
	...
	else if ((target.IsString() && (params.size() == 1)))
	{
		...
		if (p0.IsPositiveInteger())
		{
			...
			if (target.IsUTF8String())
			{
				...
				internal_index = (int)utf8_byte_position_from_index((unsigned char*)str_val.c_str(), index); // cppcheck-suppress cstyleCast
			}
		...
		else if (p0.IsPositiveIntegralVector())
		{
			...
			if (p0.IsLinearRange() && (mtx->M() == 1))
			{
				double first = (*indices)(0);
				double last = (*indices)(indices->Size()-1);

				int internal_first = (int)(first-1);
				int internal_last  = (int)(last-1);
				
				if (utf8_check)
				{
					const double* real_data = mtx->GetRealData();
					internal_first = (int)utf8_byte_position_from_index(real_data, (size_t)(first - 1));
					internal_last = (int)utf8_byte_position_from_index(real_data, (size_t)(last - 1));
					int last_size = (int)utf8_get_char_size(&real_data[internal_last]);
					internal_last += (last_size - 1);
				}

				int size = internal_last - internal_first + 1;

				hwMatrix* ret = allocateMatrix(1, size, true);
				hwMathStatus stat = ret->ReadSubmatrix(0, internal_first, 1, size, *mtx);
		...
		else if (p0.IsColon())
		{
			...
			if (mtx)
			{
				ret = allocateMatrix(mtx->Size(), 1, mtx->IsReal());
	...
	else if (target.IsStruct())
	{
		if (params.size() == 1)
		{
			...
			{
				const StructData* sd = target.Struct();
	...
	{
		if (HasOverloadedFunction(target, "subsref"))
		{
			std::string class_name = target.GetClassname();
			//const ClassInfo* ci = (*class_info_map)[class_name];

			std::vector<Currency> local_params;

			local_params.push_back(target);

			StructData* sd = new StructData();
			sd->DimensionNew(1, 1);
			sd->SetValue(0, 0, "type", "()");

			HML_CELLARRAY* cells = new HML_CELLARRAY((int)params.size(), HML_CELLARRAY::REAL);
			...
			sd->SetValue(0, 0, "subs", cells);

			local_params.push_back(sd);

			return CallOverloadedFunction("subsref", local_params);
		}
...
{
	if (p0.IsObject() || p1.IsObject())
	{
		std::vector<Currency> temp_vec;
		temp_vec.push_back(p0);
		temp_vec.push_back(p1);

		return VariableIndex(target, temp_vec);
	}
		...
		else if (p0.IsVector() || p1.IsVector())
		{
			...
			if (p0.IsPositiveIntegralVector())
			{
				if (p0.IsLinearRange())
				{
					...
					if (!p1.IsLinearRange() && !p1.IsScalar())
						first_range = p0.Vector();
				}
				else
				{
			...
			if (p1.IsPositiveIntegralVector())
			{
				if (p1.IsLinearRange())
				{
					...
					if (!p0.IsLinearRange() && !p0.IsScalar())
			...
			{
				if (!first_range_linear)
				{
					hwMatrix* ret = allocateMatrix((int)first_range.size(), (int)data->N(), data->IsReal());

					hwMatrix* temp = allocateMatrix(1, (int)data->N(), data->IsReal());

					for (int j = 0; j < first_range.size(); ++j)
					{
						hwMathStatus stat = data->ReadRow((int)first_range[j] - 1, *temp);

						if (!stat.IsOk())
							throw OML_Error(HW_ERROR_INDEXRANGE);

						ret->WriteSubmatrix(j, 0, *temp);
					}
			...
			if (first_range_linear && second_range_linear)
			{
				const hwMatrix* first  = p0.ConvertToMatrix();
				const hwMatrix* second = p1.ConvertToMatrix();
				...
				hwMathStatus stat = ret->ReadSubmatrix(start_row, start_col, first->Size(), second->Size(), *data);
				if (!stat.IsOk())
					throw OML_Error(HW_ERROR_INDEXRANGE);
			}
			else if (first_range_linear && p1.IsColon())
			{
				hwMatrix* ret = allocateMatrix((int)data->M(), (int)second_range.size(), data->IsReal());

				hwMathStatus stat = ret->ReadSubmatrix((int)first_range[0] - 1, 0, (int)first_range.size(), data->N(), *data);
				if (!stat.IsOk())
					throw OML_Error(HW_ERROR_INDEXRANGE);

				return ret;
			}
			else if (p0.IsColon() && second_range_linear)
			{
				hwMatrix* ret = allocateMatrix((int)data->M(), (int)second_range.size(), data->IsReal());

				hwMathStatus stat = ret->ReadSubmatrix(0, (int)second_range[0] - 1, data->M(), (int)second_range.size(), *data);
				if (!stat.IsOk())
					throw OML_Error(HW_ERROR_INDEXRANGE);
	...
	else if (target.IsObject())
	{
		...
		{
			std::string class_name = target.GetClassname();
			std::vector<Currency> temp_params;

			temp_params.push_back(target);

			StructData* sd = new StructData();
			sd->DimensionNew(1, 1);
			sd->SetValue(0, 0, "type", "()");

			HML_CELLARRAY* cells = new HML_CELLARRAY(2, HML_CELLARRAY::REAL);
			...
			temp_params.push_back(sd);

			return CallOverloadedFunction("subsref", temp_params);
		}
		else if (p0.IsScalar() && p1.IsScalar())
		{
			StructData* sd = target.Struct();
	                                             ...
	                                             const std::string& preload_name, const Currency& preload_val)
{
	...
	private:
		...
		NestedFunctionHelper() : _eval(nullptr)	{}
	};
	...
	else
	{
		...
		// This isn't great.  We could change this to run the statements only if nested_functions is empty and 
		// register them with the current memory scope otherwise.
		// Alternatively, we could remove the nested functions from the memory scope altogether and change how we find 
		// those functions, but that's a bigger tear-up.
		...
		if (statements)
		{
			...
			if (fi->IsConstructor())
			{
				...
				if (ret_vals.size() == 1) // should always be true for a constructor
				{
					...
					{
						const ClassInfo* ci = (*class_info_map)[fi->FunctionName()];
						Currency cur = ci->CreateEmpty();
			...
			bool old_boc = msm->GetCurrentScope()->break_on_continue;
			msm->GetCurrentScope()->break_on_continue = false;
			...
			msm->GetCurrentScope()->break_on_continue = old_boc;
		}
		...
		if (!fi->IsAnonymous() || return_values.size())
		{
...
std::string ExprTreeEvaluator::GetHelpModule(const std::string& func_name, const bool& loadLibraryBrowser)
{
	...
	else
	{
		...
		if (fi)
		{
			...
			std::string alias = GetLibraryAlias(base_dir);
			...
			// For Compose installed or workspace libraries
			std::string library_name;
			std::replace(base_dir.begin(), base_dir.end(), '\\', '/');
			std::string check_string;
			check_string = "hwx/scripts/oml/";
			std::string::size_type start_pos1 = base_dir.find(check_string);
#if _DEBUG
			if (start_pos1 == std::string::npos)
			{
				check_string = "unity/scripts/oml/";
				start_pos1 = base_dir.find(check_string);
			}
#endif
			if (start_pos1 != std::string::npos)
			{
				start_pos1 += check_string.length();

				size_t library_name_terminator = base_dir.find("/", start_pos1);
				library_name = base_dir.substr(start_pos1, library_name_terminator - start_pos1);
				if (library_name.size() && loadLibraryBrowser == false)
				{
					return library_name;
				}
			}
			...
			else // possibly this is a subfolder in a library
			{
				...
				if (scr_location != std::string::npos)
				{
					...
					if ((test_string == "scripts\\oml") || (test_string == "scripts/oml"))
					{
						...
						rhs_result = rhs_result.substr(0, rhs_lastslash); // cppcheck-suppress uselessCallsSubstr

						if (result != "hwx" && result != "unity")
						{
...
static bool IsAComposeLibrary(const std::string& libpath)
{
    std::string loaderfile=libpath+"loader.oml";
    bool existloader=FileExists(loaderfile.c_str());
    if (!existloader){
        loaderfile=libpath+"loader.omc";
        existloader=FileExists(loaderfile.c_str());
    }
    if (existloader) {
        /*std::string libini=libpath+"/library.ini";
          if (FileExists(libini.c_str()))*/
        return true;
    }
    return false;
}
...
{
    // Return the local help directory path for external libraries added using Library Manager
	std::string helpdir;
	...
	if (std_functions->find(func_name) != std_functions->end())
	{
		...
		if (fi)
		{
			...
			// For Compose installed or workspace libraries
			std::string library_name;
			std::replace(lower_case_filename.begin(), lower_case_filename.end(), '\\', '/');
			std::string check_string;
			check_string = "hwx/scripts/oml/";
			std::string::size_type start_pos1 = lower_case_filename.find(check_string);
			if (start_pos1 == std::string::npos)
			{
				check_string = "unity/scripts/oml/";
				start_pos1 = lower_case_filename.find(check_string);
			}
			if (start_pos1 != std::string::npos)
			{
				start_pos1 += check_string.length();

				size_t library_name_terminator = lower_case_filename.find("/", start_pos1);
				library_name = filename.substr(start_pos1, library_name_terminator - start_pos1);
        ...
        // check if the path is a compose library
        if (IsAComposeLibrary(helpdir))
            use_fullpath=true;
			}
...
{
	paths                 = source->paths;
	hidden_paths          = source->hidden_paths;
	restore_paths         = source->restore_paths;
	restore_hidden_paths  = source->restore_hidden_paths;
	treat_as_builtin_list = source->treat_as_builtin_list;
	cached_path_contents  = source->cached_path_contents;
...
Currency ExprTreeEvaluator::UnaryOperator(OMLTree* tree) // cppcheck-suppress constParameterPointer
{
	int	op = tree->GetType();

	OMLTree* child = tree->GetChild(0); // cppcheck-suppress constParameterPointer
...
Currency ExprTreeEvaluator::BinaryOperator(OMLTree* tree) // cppcheck-suppress constParameterPointer
{
	class NargoutHelper
	{
	public:
	...
	private:
		...
		NargoutHelper() : my_eval(nullptr), my_val(0) {}
	};
	...
	{
		const OMLTree* extra_node = tree->GetChild(1);
...
Currency ExprTreeEvaluator::AddOperator(const Currency& op1,const Currency& op2)
{
	...
	else if (op1.IsMatrix() && op2.IsMatrix())
	{
		...
		catch (hwMathException & except)
		{
			try
			{
				hwMatrix* e1 = nullptr;
				hwMatrix* e2 = nullptr;
				Currency::ExpandMatrixPair(*m1, *m2, e1, e2);
				MKLutilitiesD::Add(*e1, *e2, *ret);
				delete e1;
				delete e2;
				return ret;
			}
			catch (hwMathException & except)
			{
				...
				if (except.Status() == HW_MATH_ERR_ALLOCFAILED)
					throw OML_Error(except.Status());
				else
	...
	else if (op1.IsSparse() && op2.IsSparse())
	{
		...
		MKLutilitiesD::SparseAdd(*m1, *m2, *ret);
	...
	else if (op1.IsNDMatrix() || op2.IsNDMatrix())
	{
		...
		catch (OML_Error& except)
		{
			try
			{
				...
				if (op1.IsNDMatrix() && op2.IsNDMatrix())
				{
					const hwMatrixN* m1 = op1.MatrixN();
					const hwMatrixN* m2 = op2.MatrixN();
					Currency::ExpandMatrixPair(*m1, *m2, e1, e2);
				}
				...
				else if (op1.IsNDMatrix() && op2.IsMatrix())
				{
					const hwMatrixN* m1 = op1.MatrixN();
					const hwMatrix* m2 = op2.Matrix();
					Currency::ExpandMatrixPair(*m2, *m1, e2, e1);
				}
				else
				{
				...
				return oml_MatrixNUtil7(Currency(e1), Currency(e2), &ExprTreeEvaluator::AddOperator);
			}
			catch (OML_Error& except)
			{
				if (except.Status() == HW_MATH_ERR_ALLOCFAILED)
...
Currency ExprTreeEvaluator::SubtractOperator(const Currency& op1, const Currency& op2)
{
	...
	else if (op1.IsSparse() && op2.IsSparse())
	{
		...
		hwMatrixS* ret = new hwMatrixS;
		MKLutilitiesD::SparseAdd(*m1, temp, *ret);
	...
	else if (op1.IsNDMatrix() || op2.IsNDMatrix())
	{
		...
		catch (OML_Error& except)
		{
			try
			{
				...
				return oml_MatrixNUtil7(Currency(e1), Currency(e2), &ExprTreeEvaluator::SubtractOperator);
			}
			catch (OML_Error& except)
			{
				if (except.Status() == HW_MATH_ERR_ALLOCFAILED)
...
Currency ExprTreeEvaluator::MultiplyOperator(const Currency& op1, const Currency& op2)
{
	...
	else if (op1.IsSparse() && op2.IsSparse())
	{
		...
		hwMatrixS* ret = new hwMatrixS();
		MKLutilitiesD::SparseMult(*m1, *m2, *ret);
		return ret;
	}
	else if (op1.IsSparse() && op2.IsScalar())
	{
...
Currency ExprTreeEvaluator::EntrywiseMultiplyOperator(const Currency& op1, const Currency& op2)
{
	...
	else if (op1.IsMatrix() && op2.IsMatrix())
	{
		...
		try
		{
			MKLutilitiesD::MultByElems(*m1, *m2, *ret);
			return ret;
		}
		catch (hwMathException& except)
		{
			try
			{
				...
				hwMatrix* e2 = nullptr;
				Currency::ExpandMatrixPair(*m1, *m2, e1, e2);
				MKLutilitiesD::MultByElems(*e1, *e2, *ret);
				delete e1;
				delete e2;
				return ret;
			}
	...
	else if (op1.IsNDMatrix() || op2.IsNDMatrix())
	{
		...
		catch (OML_Error& except)
		{
			try
			{
				...
				return oml_MatrixNUtil7(Currency(e1), Currency(e2), &ExprTreeEvaluator::EntrywiseMultiplyOperator);
			}
			catch (OML_Error& except)
			{
				if (except.Status() == HW_MATH_ERR_ALLOCFAILED)
...
Currency ExprTreeEvaluator::DivideOperator(const Currency& op1, const Currency& op2)
{
	...
	else if (op1.IsMatrix() && op2.IsSparse())
	{
		...
		hwMatrix* ret = allocateMatrix();
		MKLutilitiesD::SparseDivideRight(*m1, *m2, *ret);
		return ret;
	}
	else if (op1.IsSparse() && op2.IsSparse())
	{
		...
		hwMatrix* ret = allocateMatrix();
		MKLutilitiesD::SparseDivideRight(m1full, *m2, *ret);
		return ret;
	}
	...
	else if (op1.IsScalar() && op2.IsMatrix())
	{
		...
		{
			hwMatrix* ret = allocateMatrix();
			hwMatrix* temp = allocateMatrix(1, 1, true);
	...
	else if (op1.IsComplex() && op2.IsMatrix())
	{
		...
		{
			hwMatrix* ret = allocateMatrix();
			hwMatrix* temp = allocateMatrix(1, 1, false);
...
Currency ExprTreeEvaluator::EntrywiseDivideOperator(const Currency& op1, const Currency& op2)
{
	...
	else if (op1.IsMatrix() && op2.IsMatrix())
	{
		...
		try
		{
			MKLutilitiesD::DivideByElems(*m1, *m2, *ret);
			return ret;
		}
		catch (hwMathException & except)
		{
			try
			{
				...
				hwMatrix* e2 = nullptr;
				Currency::ExpandMatrixPair(*m1, *m2, e1, e2);
				MKLutilitiesD::DivideByElems(*e1, *e2, *ret);
				delete e1;
				delete e2;
				return ret;
			}
	...
	else if (op1.IsNDMatrix() || op2.IsNDMatrix())
	{
		...
		catch (OML_Error& except)
		{
			try
			{
				...
				return oml_MatrixNUtil7(Currency(e1), Currency(e2), &ExprTreeEvaluator::EntrywiseDivideOperator);
			}
			catch (OML_Error& except1)
			{
				if (except1.Status() == HW_MATH_ERR_ALLOCFAILED)
					throw OML_Error(except1.Status());
				else
					throw OML_Error(OML_ERR_ARRAYSIZE);
			}
		}
...
Currency ExprTreeEvaluator::LeftDivideOperator(const Currency& op1, const Currency& op2)
{
    ...
    else if (op1.IsSparse() && op2.IsMatrix())
    {
        ...
        hwMatrix* ret = allocateMatrix();
		MKLutilitiesD::SparseDivideLeft(*m1, *m2, *ret);
        return ret;
    }
    else if (op1.IsSparse() && op2.IsSparse())
    {
        ...
        hwMatrix* ret = allocateMatrix();
		MKLutilitiesD::SparseDivideLeft(*m1, m2full, *ret);
        return ret;
    }
...
Currency ExprTreeEvaluator::EntrywiseLeftDivideOperator(const Currency& op1, const Currency& op2)
{
    ...
    else if (op1.IsMatrix() && op2.IsMatrix())
    {
        ...
        try
        {
			MKLutilitiesD::DivideByElems(*m2, *m1, *ret);
            return ret;
        }
        catch (hwMathException & except)
        {
            try
            {
				...
				hwMatrix* e2 = nullptr;
				Currency::ExpandMatrixPair(*m1, *m2, e1, e2);
				MKLutilitiesD::DivideByElems(*e2, *e1, *ret);
                delete e1;
                delete e2;
                return ret;
            }
	...
	else if (op1.IsNDMatrix() || op2.IsNDMatrix())
	{
		...
		catch (OML_Error& except)
		{
			try
			{
				...
				return oml_MatrixNUtil7(Currency(e1), Currency(e2), &ExprTreeEvaluator::EntrywiseLeftDivideOperator);
			}
			catch (OML_Error& except)
			{
				if (except.Status() == HW_MATH_ERR_ALLOCFAILED)
...
Currency ExprTreeEvaluator::EqualityOperatorEx(const Currency& op1, const Currency& op2, int oper)
{
	...
	{
		const hwMatrix* m2 = op2.Matrix();
		hwMatrix* result = allocateMatrix(m2->M(), m2->N(), true);
	...
	else if (op1.IsMatrixOrString() && op2.IsScalar())
	{
		...
		hwMatrix* result = allocateMatrix(m1->M(), m1->N(), true);
	...
	else if (op1.IsNDMatrix() || op2.IsNDMatrix())
	{
		...
		catch (OML_Error& except)
		{
			try
			{
				...
				return oml_MatrixNUtil7(Currency(e1), Currency(e2), &ExprTreeEvaluator::EqualityOperatorEx);
			}
			catch (OML_Error& except)
			{
				if (except.Status() == HW_MATH_ERR_ALLOCFAILED)
	...
	else if (op1.IsMatrixOrString() && op2.IsMatrixOrString())
	{
		...
		{
			hwMatrix* result = allocateMatrix(m1->M(), m1->N(), m1->IsReal());
	...
	{
		hwComplex cmplx = op1.Complex();
		const hwMatrixS* m2 = op2.MatrixS();
	
		// check cases
		if (m2->IsReal())
		{
			hwMatrixS* m2c = new hwMatrixS;
			m2c->PackComplex(*m2);
			return EqualityOperatorEx(op1, m2c, oper);
		}
	
		// extract sparse data for comparison
		int m = m2->M();
		int n = m2->N();
		...
		m2->NZinfo(0, nnz2 - 1, row2, col2, value2);
	
		// build comparison matrix
		std::vector<int> colCount(n + 1);
		...
		for (int j = 0; j < n; ++j)
		{
			for (int i = 0; i < m; ++i)
			{
				if (row2[idx2] == i && col2[idx2] == j)
				{
					if (cmplx == value2.z(idx2))
					{
						++colCount[j + 1];
						rowNumber.push_back(i);
						value.push_back(1.0);
					}
		...
		hwMatrixS* result = new hwMatrixS(m, n, colCount.data(),
			colCount.data() + 1, rowNumber.data(), value.data());
	
		return result;
	}
	else if (op1.IsSparse() && op2.IsComplex())
	{
		return EqualityOperatorEx(op2, op1, oper);
	}
	else if (op1.IsScalar() && op2.IsSparse())
	{
		double scalar = op1.Scalar();
		const hwMatrixS* m2 = op2.MatrixS();

		// check cases
		bool realCase = m2->IsReal() ? true : false;
		...
		for (int j = 0; j < n; ++j)
		{
			for (int i = 0; i < m; ++i)
			{
				if (row2[idx2] == i && col2[idx2] == j)
				{
					if ( realCase && (scalar == value2(idx2)) ||
						!realCase && (value2.z(idx2) == scalar))
					{
						++colCount[j + 1];
						rowNumber.push_back(i);
						value.push_back(1.0);
	...
	else if (op1.IsSparse() && op2.IsSparse())
	{
		...
		if (m1->IsReal())
		{
			if (!m2->IsReal())
			{
				hwMatrixS* m1c = new hwMatrixS;
				m1c->PackComplex(*m1);
				return EqualityOperatorEx(m1c, op2, oper);
			}
		}
		else if (m2->IsReal())
		{
		...
		m1->NZinfo(0, nnz1 - 1, row1, col1, value1);
		m2->NZinfo(0, nnz2 - 1, row2, col2, value2);
		...
		for (int j = 0; j < n; ++j)
		{
			for (int i = 0; i < m; ++i)
			{
				if (row1[idx1] == i && col1[idx1] == j)
				{
					if (row2[idx2] == i && col2[idx2] == j)
					{
						if (( realCase && (value1(idx1) == value2(idx2))) ||
							(!realCase && (value1.z(idx1) == value2.z(idx2))))
						{
							++colCount[j + 1];
							rowNumber.push_back(i);
							value.push_back(1.0);
					...
					{
						if (( realCase && value1(idx1) == 0.0) ||
							(!realCase && value1.z(idx1) == 0.0))
						{
							++colCount[j + 1];
							rowNumber.push_back(i);
							value.push_back(1.0);
				...
				else if (row2[idx2] == i && col2[idx2] == j)
				{
					if (( realCase && value2(idx2) == 0.0) ||
						(!realCase && value2.z(idx2) == 0.0))
					{
						++colCount[j + 1];
						rowNumber.push_back(i);
						value.push_back(1.0);
	...
	else if (op1.IsMatrix() && op2.IsSparse())
	{
		...
		for (int j = 0; j < n; ++j)
		{
			for (int i = 0; i < m; ++i)
			{
				if (row2[idx2] == i && col2[idx2] == j)
				{
					if ((realCase && ((*m1)(i, j) == value2(idx2))) ||
						(!realCase && (m1->z(i, j) == value2.z(idx2))))
					{
						++colCount[j + 1];
						rowNumber.push_back(i);
						value.push_back(1.0);
				...
				{
					if ((realCase && (*m1)(i, j) == 0.0) ||
						(!realCase && m1->z(i, j) == 0.0))
					{
						++colCount[j + 1];
						rowNumber.push_back(i);
						value.push_back(1.0);
...
Currency ExprTreeEvaluator::LessThanOperator(const Currency& op1, const Currency& op2)
{
	...
	else if (op1.IsNDMatrix() || op2.IsNDMatrix())
	{
		...
		catch (OML_Error& except)
		{
			try
			{
				...
				return oml_MatrixNUtil7(Currency(e1), Currency(e2), &ExprTreeEvaluator::LessThanOperator);
			}
			catch (OML_Error& except)
			{
				if (except.Status() == HW_MATH_ERR_ALLOCFAILED)
	...
	{
		return CallOverloadedOperator("lt", op1, op2);
	}
	else
	{
		throw OML_Error(HW_ERROR_INCOMPDIM);
...
Currency ExprTreeEvaluator::GreaterThanOperator(const Currency& op1, const Currency& op2)
{
	...
	else if (op1.IsNDMatrix() || op2.IsNDMatrix())
	{
		...
		catch (OML_Error& except)
		{
			try
			{
				...
				return oml_MatrixNUtil7(Currency(e1), Currency(e2), &ExprTreeEvaluator::GreaterThanOperator);
			}
			catch (OML_Error& except1)
			{
				if (except1.Status() == HW_MATH_ERR_ALLOCFAILED)
	...
	{
		return CallOverloadedOperator("gt", op1, op2);
	}
	else
	{
		throw OML_Error(HW_ERROR_INCOMPDIM);
...
Currency ExprTreeEvaluator::LessEqualOperator(const Currency& op1, const Currency& op2)
{
	...
	else if (op1.IsNDMatrix() || op2.IsNDMatrix())
	{
		...
		catch (OML_Error& except)
		{
			try
			{
				...
				return oml_MatrixNUtil7(Currency(e1), Currency(e2), &ExprTreeEvaluator::LessEqualOperator);
			}
			catch (OML_Error& except)
			{
				if (except.Status() == HW_MATH_ERR_ALLOCFAILED)
	...
	{
		return CallOverloadedOperator("le", op1, op2);
	}
	else
	{
		throw OML_Error(HW_ERROR_INCOMPDIM);
...
Currency ExprTreeEvaluator::GreaterEqualOperator(const Currency& op1, const Currency& op2)
{
	...
	else if (op1.IsNDMatrix() || op2.IsNDMatrix())
	{
		...
		catch (OML_Error& except)
		{
			try
			{
				...
				return oml_MatrixNUtil7(Currency(e1), Currency(e2), &ExprTreeEvaluator::GreaterEqualOperator);
			}
			catch (OML_Error& except)
			{
				if (except.Status() == HW_MATH_ERR_ALLOCFAILED)
	...
	{
		return CallOverloadedOperator("ge", op1, op2);
	}
	else
	{
		throw OML_Error(HW_ERROR_INCOMPDIM);
...
Currency ExprTreeEvaluator::LogicalOperator(OMLTree* tree)
{
	...
	{
		if (op == AND)
			return CallOverloadedOperator("and", op1, op2);
		else if (op == OR)
			return CallOverloadedOperator("or", op1, op2);
		else
			throw OML_Error(HW_ERROR_UNSUPOP);
	}
...
Currency ExprTreeEvaluator::PowOperator(const Currency& op1, const Currency& op2)
{
	if (op1.IsScalar() && op2.IsScalar())
	{
		...
		if (base < 0.0)
		{
			if (isint(exponent))
				...
				return hwComplex::pow_c(base, exponent);
		}
		else
		{
			return pow(base, exponent);
	...
	{
		hwComplex result;
		result = hwComplex::pow(op1.Complex(), op2.Complex());
		return result;
	}
	else if (op1.IsMatrix() && op2.IsScalar())
	{
		// select either controls toolbox (SLICOT) or Eigen algorithm
		if (op2.IsInteger())
		{
			// try kernel algorithm
			bool useControlToolbox = false;
			const hwMatrix* m1 = op1.Matrix();
			std::unique_ptr<hwMatrix> result(EvaluatorInterface::allocateMatrix());

			hwMathStatus status = result->Power(*m1, op2.Scalar());

			if (!status.IsOk())
			{
				status.ResetArgs();   // suppress argument numbers because A^b has no argument list

				if (status.IsWarning())
				{
					useControlToolbox = true;
				}
				else
				{
			...
			if (useControlToolbox)
			{
				// switch to controls toolbox (SLICOT)
				Currency out;
				...
				try
				{
					...
					Currency temp = CallFunction("logm", inputs2);
					hwMatrix* logM = temp.GetWritableMatrix();
					logM->MultEquals(op2.Scalar());
					inputs2.clear();
					inputs2.push_back(temp);
					out = CallFunction("expm", inputs2);
					return out;
				}
				catch (OML_Error& err)
				{
					if (err.Status().IsWarning())
					{
						builtin_error_scope = "";
						EvaluatorInterface eval(this);
						BuiltInFuncsUtils::SetWarning(eval, err.Status().GetMessage());
						return out;
					}
				}
			}

			// return kernel algorithm result
			if (status.IsWarning())
			{
				EvaluatorInterface eval(this);
				BuiltInFuncsUtils::SetWarning(eval, status.GetMessage());
			}
		...
		else	// non-integer exponent
		{
			...
			try
			{
				// try controls toolbox (SLICOT)
				std::vector<Currency> inputs2;
				inputs2.push_back(op1);
				Currency temp = CallFunction("logm", inputs2);
				hwMatrix* logM = temp.GetWritableMatrix();
				logM->MultEquals(power);
				inputs2.clear();
				inputs2.push_back(temp);
				return CallFunction("expm", inputs2);
			}
			catch (OML_Error& err)
			{
				if (err.Status().GetMsgCode() != HW_MATH_ERR_NONE)
				{
					builtin_error_scope = "";
				...
				// else toolbox not present
			}

			// revert to kernel algorithm
			const hwMatrix* m1 = op1.Matrix();
			std::unique_ptr<hwMatrix> result(EvaluatorInterface::allocateMatrix());

			hwMathStatus status = result->Power(*m1, power);
	...
	{
		hwComplex power = op2.Complex();

		if (isnan(power.Real()) || isnan(power.Imag()))
		{
			const hwMatrix* m1 = op1.Matrix();
			return allocateMatrix(m1->M(), m1->N(), std::numeric_limits<double>::quiet_NaN());
		}
	...
	else if (op1.IsScalar() && op2.IsMatrix())
	{
		...
		try
		{
			...
			status = result->Mult(*m2, log(op1.Scalar()));
			std::vector<Currency> inputs2;
			inputs2.push_back(result);
			Currency powM = CallFunction("expm", inputs2);
			return powM;
		}
		catch (OML_Error&)
		{
			// revert to kernel algorithm
			hwMatrix* result = allocateMatrix();
			status = result->MatExp((*m2) * log(op1.Scalar()));
...
Currency ExprTreeEvaluator::EntrywisePowOperator(const Currency& op1, const Currency& op2)
{
	...
	else if (op1.IsMatrix() && op2.IsScalar())
	{
		...
		hwMatrix* result = allocateMatrix();
		MKLutilitiesD::PowerByElems(*m1, exponent, *result);
        return result;
    }
    else if (op1.IsMatrix() && op2.IsComplex())
    {
    ...
    else if (op1.IsScalar() && op2.IsMatrix())
	{
		...
		if (m1->IsReal())
		{
			...
			if (base < 0.0)
			{
				result = allocateMatrix(m1->M(), m1->N(), false);

				for (int j = 0; j < m1->Size(); j++)
					result->z(j) = hwComplex::pow_c(op1.Scalar(), (*m1)(j));
			}
			else
			{
				result = allocateMatrix(m1->M(), m1->N(), true);
	...
	else if (op1.IsMatrix() && op2.IsMatrix())
	{
        ...
        try
        {
			MKLutilitiesD::PowerByElems(*m1, *m2, *result);
            return result;
        }
        catch (hwMathException& except)
        {
            ...
            if (except.Status() == HW_MATH_ERR_ARRAYSIZE)
            {
                ...
                try
                {
					MKLutilitiesD::PowerByElems(*e1, *e2, *result);
                    delete e1;
                    delete e2;
                    return result;
                }
	...
	else if (op1.IsNDMatrix() || op2.IsNDMatrix())
	{
		...
		catch (OML_Error& except)
		{
			try
			{
				...
				return oml_MatrixNUtil7(Currency(e1), Currency(e2), &ExprTreeEvaluator::EntrywisePowOperator);
			}
			catch (OML_Error& except)
			{
				if (except.Status() == HW_MATH_ERR_ALLOCFAILED)
...
Currency ExprTreeEvaluator::NotOperator(const Currency& op)
{
	...
	else if (op.IsMatrix() || op.IsString())
	{
		...
		hwMatrix* ret = allocateMatrix(mtx->M(), mtx->N(), true);
...
Currency ExprTreeEvaluator::PostFunctionIndexHelper(const Currency& target, OMLTree* index_tree)
{
	...
	if (index_tree->GetType() == PARAM_LIST)
	{
		...
		end_context_varname  = NULL;
		end_context_currency = (Currency*)&target; // cppcheck-suppress cstyleCast 
...
Currency ExprTreeEvaluator::FunctionCall(OMLTree* tree)
{
	...
	{
		if (!tree->u)
			tree->u = (void*)Currency::vm.GetStringPointer(func->GetText()); // cppcheck-suppress cstyleCast 
	...
	else if (func->GetType() == STRUCT)
	{
		...
		std::string sc_text = struct_child->GetText();

		if (!msm->Contains(Currency::vm.GetStringPointer(sc_text)))
		{
			ClassInfo* ci = NULL;
			...
			std::string oml_file_name = struct_child->GetText() + ".oml";

			if (!ci)
			{
				if (FindFileInPath(oml_file_name, found_name))
				{
					ParseAndRunFile(found_name, false);
		...
		if (func_args->GetType() == PARAM_LIST)
		{
			...
			if (!val.IsNothing())
			{
				...
				else
				{
					...
					int          old_index    = end_context_index;
			...
			else if (var_ptr)
			{
				...
				catch (OML_Error& e) // handle errors in parameter evaluation, typically end
				{
					...
					{
						std::string error_message = "Error: '" + *var_ptr + "' is not defined";
						throw OML_Error(error_message);
					}
					else
					{
				...
				{
					function_args.push_back(func_args);
					Currency ret = CallFunction(var_ptr, param_vals);
...
void ExprTreeEvaluator::DoMultiReturnFunctionCall(FUNCPTR fptr, const std::string& func_name, const std::vector<Currency>& param_values, int num_ins, bool suppress_output, OMLTree* out_tree)
{
	...
	int  nargout_value = num_rets;
	bool is_broadcast  = false;

	// check for a broadcast output
	if (num_rets == 1)
	{
		OMLTree* child_1 = out_tree->GetChild(0);

		if ((child_1->GetType() == CELL_VAL) && (child_1->ChildCount() > 1))
		{
			OMLTree* child_2 = child_1->GetChild(0);
			OMLTree* child_3 = child_1->GetChild(1);

			if ((child_2->GetType() == IDENT) && (child_3->GetType() == PARAM_LIST))
			{
				if (child_3->ChildCount() == 1)
				{
					OMLTree* child_4 = child_3->GetChild(0);

					if (child_4->GetType() == COLON)
					{
						Currency temp = msm->GetValue(child_2->GetText());
						nargout_value = temp.CellArray()->Size();
						is_broadcast = true;
					}
				}
			}
	...
	PushNargValues(num_ins, nargout_value);
	...
	for (int j=0; j<num_rets; j++)
	{
		// if it's a broadcast output, convert the ret values into a CELL_LIST
		Currency loop_cur = ret[j];
		loop_cur.ClearOutputName();

		if (num_rets == 1 && is_broadcast)
		{
			HML_CELLARRAY* cell_list = new HML_CELLARRAY((int)ret.size(), HML_CELLARRAY::REAL);

			for (int k = 0; k < ret.size(); ++k)
				(*cell_list)(k) = ret[k];

			loop_cur = cell_list;
			loop_cur.SetMask(Currency::MASK_CELL_LIST);
		}
...
void ExprTreeEvaluator::DoMultiReturnFunctionCall(ALT_FUNCPTR alt_fptr, const std::string& func_name, const std::vector<Currency>& param_values, int num_ins, bool suppress_output, OMLTree* out_tree)
{
	...
	for (int j=0; j<num_rets; j++)
	{
		const OMLCurrencyImpl* loop_cur_ptr = (const OMLCurrencyImpl*)out_list->Get(j); // cppcheck-suppress cstyleCast 
...
std::vector<Currency> ExprTreeEvaluator::DoAnonymousMultiReturnFunctionCall(FunctionInfo* fi, const std::vector<Currency>& param_values, int num_ins, int num_rets, bool suppress_output, std::vector<const std::string*>* out_vars)
{
	...
	if (!FindFunctionByName(my_func, &inner_fi, &fptr, &aptr))
		throw OML_Error(HW_ERROR_UNKNOWNFUN);
...
void ExprTreeEvaluator::DoMultiReturnFunctionCall(FunctionInfo* fi, const std::vector<Currency>& param_values, int num_ins, bool suppress_output, OMLTree* out_tree)
{
	...
	if (fi->IsAnonymous())
	{
		...
		for (size_t j=0; j<num_rets; j++)
		{
			...
			if (!out_var->u)
				out_var->u = (void*)Currency::vm.GetStringPointer(out_var->GetText());	 // cppcheck-suppress cstyleCast
	...
	if (IsInterrupt())
	{
		return; // Don't throw errors if interpreter is interrupted
	}
	...
	{
		OMLTree* name_tree = out_tree->GetChild(0);
		_lhs_eval = true; // redundantAssignment
		Currency target = RUN(name_tree);
		_lhs_eval = false;
	...
	for (int j=0; j<num_rets; j++)
	{
		...
		{ 
			...
			if (child_n->GetType() != NEGATE) // don't run ~
			{
				...
				{
					const OMLTree* param_tree = child_n->GetChild(1);
					...
					if (param_tree->ChildCount() == 1)
					{
						...
						if (test.IsPositiveIntegralVector())
						{
							...
							_lhs_eval = true; // cppcheck-suppress redundantAssignment
							Currency target = RUN(name_tree);
							_lhs_eval = false;
...
Currency ExprTreeEvaluator::MultiReturnFunctionCall(OMLTree* tree)
{
	...
	{
		const OMLTree* func_tree = func->GetChild(0);	
	...
	{
		if (!func->u)
			func->u = (void*)Currency::vm.GetStringPointer(func->GetText()); // cppcheck-suppress cstyleCast
	}
	...
	const std::string* my_func = (const std::string*)func->u;  // cppcheck-suppress cstyleCast
	...
	if (cur_0.IsObject())
	{
		if (HasOverloadedFunction(cur_0, *my_func))
		{ 
			std::string class_name = cur_0.GetClassname();
			ClassInfo*  ci         = (*class_info_map)[class_name];
	...
	for (int j=0; j<num_params; j++)
	{
		...
		else
		{
			if ((j == 0) && cur_0_valid)
			{
				if (cur_0.IsCellList())
				{
					HML_CELLARRAY* cells = cur_0.CellArray();
	...
	if (func_tree->ChildCount() == 2)
	{
		...
		if (params_tree->GetType() == PARAM_LIST)
		{
			...
			else if (cur.IsObject() || cur.IsPointer())
			{
				...
				if (ci->IsSubclassOf("handle"))
				{
					// don't allow pointers to pointers
					if (cur.IsPointer())
						param_values.push_back(cur);
					else
						param_values.push_back(&cur);
...
Currency ExprTreeEvaluator::SwitchCase(OMLTree* tree)
{
	...
	{
		const OMLTree*    case_tree = tree->GetChild(j);
...
Currency ExprTreeEvaluator::WhileLoop(OMLTree* tree)
{
	...
	_store_suppressed  = false;
	bool old_boc       = msm->GetCurrentScope()->break_on_continue;
	msm->GetCurrentScope()->break_on_continue = true;

	while (1)
	{
		// we need to do this becuase the condition tree is not a STMT so there is no check there
		if (IsInterrupt())
...
public:
	LoopHelper()
		: _counter(0)
	    , _start (0)
	    , _incr (0) { _mtx = NULL; _num_steps = 0; _is_range = false; }
...
bool LoopHelper::IsValidRange()
{
	if (_is_range && (_num_steps > 0))
		return true;
	else
		return false;
}
...
{
	hwMatrix* new_mat = new hwMatrix;
	_mtx->ReadColumn((int)_counter, *new_mat); // cppcheck-suppress cstyleCast
	_counter++;
	return new_mat;
}
...
Currency ExprTreeEvaluator::ForLoop(OMLTree* tree)
{
	...
	if (!loop_tree->u)
		loop_tree->u = (void*)Currency::vm.GetStringPointer(loop_tree->GetText()); // cppcheck-suppress cstyleCast
	std::string* loop_var = (std::string*)loop_tree->u; // cppcheck-suppress cstyleCast
		...
		else
		{
			if (lh.IsReal())
			{
				if (lh.IsValidRange())
					...
					msm->SetValue(loop_var, allocateMatrix(1, 0, true));
			}
		}
	}
	else if (lh.IsMatrix())
...
void threadCallback(const std::string* loop_var,  LoopHelper* lh, ExprTreeEvaluator* eval, OMLTree* tree, std::string* err_str)
{
	...
	{
		if (err_str->size() == 0)
			*err_str = eval->FormatErrorMessage(e.GetErrorMessage());
	}
}
...
Currency ExprTreeEvaluator::ParforLoop(OMLTree* tree)
{
	...
	else if (test->GetType() == IDENT)
	{
		Currency range = RUN(test);

		if (range.IsMatrix())
			lh.SetMatrix(range.Matrix());
	}
	else
	{
		throw OML_Error("Unsupported parfor loop variable");
	}
	...
	hwMatrix*         mtx      = NULL;
	hwMatrixN*        nd_mtx   = NULL;
	HML_CELLARRAY*    cells    = NULL;
	HML_ND_CELLARRAY* nd_cells = NULL;
	...
	for (sliced_iter = sliced_vars.begin(); sliced_iter != sliced_vars.end(); ++sliced_iter)
	{
		...
		else if (sliced.IsNDMatrix())
		{
			throw OML_Error("ND matrices are not supported in parfor loops");
			nd_mtx = sliced.GetWritableMatrixN();
			Currency::ignore_cow_pointers.insert(nd_mtx);
		}
		...
		else if (sliced.IsNDCellArray())
		{
			throw OML_Error("ND cell arrays are not supported in parfor loops");
			nd_cells = sliced.CellArrayND();
			Currency::ignore_cow_pointers.insert(nd_cells);
		}
	}
	...
	for (int j = 0; j < num_threads; ++j)
	{
		...
		MemoryScope* thread_scope = thread_msm->GetCurrentScope();
		thread_scope->SetFunctionInfo(GetCurrentScope()->GetFunctionInfo()); // required for nested and local functions
		...
		for (sliced_iter = sliced_vars.begin(); sliced_iter != sliced_vars.end(); ++sliced_iter)
		{
			...
			{
				nd_mtx = sliced.GetWritableMatrixN();
				thread_msm->SetValue(*sliced_iter, nd_mtx);
			}
			else if (sliced.IsCellArray())
			{
				cells = sliced.CellArray();
			...
			{
				nd_cells = sliced.CellArrayND();
				thread_msm->SetValue(*sliced_iter, nd_cells);
			}
			else
			{
		...
		if (j > 0)
			...
			thread_start = loop_start;

		thread_end = ceil(thread_start + thread_range - 1);

		if (thread_end > loop_end)
			thread_end = loop_end;

		lh->SetRange(thread_start, thread_end);

		std::thread* threadObj = new std::thread(threadCallback, loop_var, lh, thread_eval, run_tree, &thread_error);
	...
	if (thread_error.size())
		throw OML_Error(thread_error, false);
...
void ExprTreeEvaluator::RegisterFunction(OMLTree* tree)
{
...
Currency ExprTreeEvaluator::FunctionDefinition(OMLTree* tree)
{
	...
	const OMLTree* subtree = tree->GetChild(last_token_idx);
	...
	if (fi)
	{
		...
		if (fi->IsNested())
		{
			...
			if (msm->GetCurrentScope()->GetFunctionInfo())
				current_file = msm->GetCurrentScope()->GetFunctionInfo()->FileName();
		...
		if (fi->IsNested())
		{
			bool accepted = msm->RegisterNestedFunction(fi);
			defined = true;

			if (!accepted)
			{
				fi->DecrRefCount();
				delete fi; // it's already been defined and we don't want to leak anything
			}
		}
		...
		if (!defined)
		{
			...
			if (IsStdFunction(fi->FunctionName())) // cppcheck-suppress deallocuse
			{
				BuiltinFunc bif = (*std_functions)[fi->FunctionName()]; // cppcheck-suppress deallocuse
				if (bif.locked)
				{
...
Currency ExprTreeEvaluator::AnonymousFunctionDefinition(OMLTree* tree)
{
	...
	if (num_children == 1)
	{
		...
		if (FindFunctionByName(func_name, &fi, &fptr, &aptr))
		{
			...
			else if (fi)
			{
				if (fi->IsNested())
				{
					FunctionInfo* copy_fi = new FunctionInfo(*fi);
					GetCurrentScope()->CacheNestedFunction(copy_fi);
					fi = copy_fi;
					fi->IncrRefCount(); // one in the cached list and one for the return value
				}
				else
				{
					fi->IncrRefCount();
	...
	else
	{
		...
		if (msm->GetCurrentScope()->GetFunctionInfo())
		{
			if (fi->local_functions && fi->local_functions->empty())
				delete fi->local_functions;
			fi->local_functions = msm->GetCurrentScope()->GetFunctionInfo()->local_functions;
		}
	}
...
{
	unsigned int num_rows = static_cast<unsigned int>(currencies.size());
	...
	*sparse_output = false;
	...
	{
		unsigned int row_cols = static_cast<unsigned int>(currencies[j].size());
		...
		for (unsigned int k=0; k<row_cols; k++)
		{
			...
			if (k == 0)
			{
				...
				{
					const StructData* sd = temp_cur.Struct();
...
// running the tree the first time :-)
	...
	for (unsigned int j=0; j<num_tree_rows; j++)
	{
		...
		const OMLTree* row_tree = tree->GetChild(j);
		...
		for (unsigned int k=0; k<num_row_cols; k++)
		{
			...
			else if (temp_cur.IsObject())
			{
				// I'm not sure if this is correct.  This may need to be altered such that one of the first two elements
				// must be an object.  Otherwise, we'll call the overloaded operator with two parameters, neither of which
				// are objects.
				if (!overloaded_class)  // the first object we find is the one in change
				{
					if (HasOverloadedFunction(temp_cur, "horzcat"))
						overloaded_class = true;
					else if (HasOverloadedFunction(temp_cur, "vertcat"))
						overloaded_class = true;
				}
			}
	...
	if (overloaded_class)
	{
		std::vector<Currency> row_results;

		for (int j = 0; j < currencies.size(); ++j)
		{
			Currency row_result = currencies[j][0];

			if (currencies[j].size() > 1)
			{
				for (int k = 1; k < currencies[j].size(); ++k)
				{
					Currency temp_cur = currencies[j][k];

					if (row_result.IsObject() || temp_cur.IsObject())
					{
						row_result = CallOverloadedOperator("horzcat", row_result, temp_cur);
					}
					else
					{
						std::vector<Currency> params;
						params.push_back(row_result);
						params.push_back(temp_cur);

						row_result = CallFunction("horzcat", params);
					}
				}
			}
		...
		Currency result = row_results[0];
		
		if (currencies.size() > 1)
		{
			for (int j = 1; j < row_results.size(); ++j)
			{
				Currency temp_cur = row_results[j];

				if (result.IsObject() || temp_cur.IsObject())
				{
					result = CallOverloadedOperator("vertcat", result, temp_cur);
				}
				else
				{
					std::vector<Currency> params;
					...
					result = CallFunction("vertcat", params);
				}
			}
		}
...
Currency ExprTreeEvaluator::StatementList(OMLTree* tree)
{
	...
	const std::string* dummy_ptr   = Currency::pm.GetStringPointer("dummy");
	const std::string* unsaved_ptr = Currency::pm.GetStringPointer("Unsaved file");
	...
	for (int i = starting_statement; i < num_statements; ++i)
	{
		...
		if (stmt->func_ptr)
		{
			// need to cache the state since statments can turn it on and off
			bool is_profiling = _profiling;

			if (is_profiling)
			{
				ProfileTimer::Initialize();

				if (profile_timers.size())
					profile_timers.back()->Stop();

				profile_timers.push_back(ProfileTimer::GetNext());
				profile_timers.back()->Start();
			}

			r = RUN(stmt);

			if (is_profiling && profile_timers.size()) // turning profiling on gets us here without hitting the "pre" code
			{
				profile_timers.back()->Stop();
				double new_val = profile_timers.back()->Time();

				const std::string* file_ptr = msm->GetCurrentScope()->GetFilenamePtr();

				if (file_ptr == dummy_ptr)
					file_ptr = unsaved_ptr;

				if (file_ptr)
				{
					std::map<int, std::tuple<double, int>>& temp_map = profile_data[file_ptr];

					DebugInfo stored_dbg = DebugInfo::DebugInfoFromTree(stmt);
					int line = stored_dbg.LineNum();
					
					if (line != 0) // happens for blank lines
					{
						std::tuple<double, int>& temp_tuple = temp_map[line];

						double& time = std::get<0>(temp_tuple);
						int& count = std::get<1>(temp_tuple);
				...
				ProfileTimer::ReleaseLast();
				profile_timers.pop_back();
		...
		if (msm->GetCurrentScope()->break_on_continue && r.IsContinue())
			break;
	}
...
Currency ExprTreeEvaluator::AssignOperator(OMLTree* tree)
{
	...
	if (!lhs->u)
	{
		lhs->u = (void*)Currency::vm.GetStringPointer(lhs->GetText()); // cppcheck-suppress cstyleCast 
		const std::string* pString = (std::string*)lhs->u;  // cppcheck-suppress cstyleCast 
	...
	if (value.IsObject())
	{
		...
		// This can happen when creating (incomplete) class objects from C++ functions
		if (!ci)
		{
			std::string file_name;
			std::string oml_file_name = value.GetClassname() + ".oml";

			if (FindFileInPath(oml_file_name, file_name))
				ParseAndRunFile(file_name, false);
			
			ci = (*class_info_map)[value.GetClassname()];

			if (!ci)
				throw OML_Error("Cannot find class definition");
		}
...
Currency ExprTreeEvaluator::AssignmentUtility(OMLTree* lhs, const Currency& new_value)
{
	...
	if (old_name && !msm->Contains(old_name))
	{
	...
	if (lhs->GetType() != FUNC)
	{
		...
		{
			const OMLTree* index_tree = lhs->GetChild(1);
		...
		catch (const OML_Error& e)
		{
			...
			const std::string* var = lhs->GetLeadingIdent();
			Currency test = msm->GetValue(old_name);

			if (test.IsObject() && HasOverloadedFunction(test, "subsasgn"))
			{
				if (lhs->GetType() == STRUCT)
				{
					int      struct_count = 1;
					OMLTree* lhs_tree     = lhs;

					while (1)
					{
						if (lhs_tree->ChildCount() && (lhs_tree->GetChild(0)->GetType() == STRUCT))
						{
							struct_count++;
							lhs_tree = lhs_tree->GetChild(0);
						}
						else
						{
							break;
					...
					std::vector<Currency> dummy;
					dummy.push_back(test);

					StructData* sd = new StructData();
					sd->DimensionNew(1, struct_count);

					lhs_tree = lhs;

					for (int j = 0; j < struct_count; ++j)
					{
						int index = struct_count - j - 1;

						std::string field_name = lhs_tree->GetChild(1)->GetText();
						sd->SetValue(0, index, "type", ".");
						sd->SetValue(0, index, "subs", field_name);
					...
					dummy.push_back(new_value);
					
					Currency overload = CallOverloadedFunction("subsasgn", dummy);

					msm->SetValue(var, overload);

					return overload;
				}
				else if (lhs->GetType() == CELL_VAL)
				{
					std::vector<Currency> dummy;
					dummy.push_back(test);
					...
					HML_CELLARRAY* cells = new HML_CELLARRAY((int)index_vec.size(), HML_CELLARRAY::REAL);

					for (int j = 0; j < index_vec.size(); ++j)
					{
						if (index_vec[j].IsColon())
							(*cells)(j) = ":";
						else
							(*cells)(j) = index_vec[j];
					}
					...
					if (!overload.IsObject())
						throw OML_Error("Subsasgn override must return an object");
		...
		{
			throw OML_Error("Invalid LHS");
		}

		if (new_value.IsCellList())
		{
			HML_CELLARRAY* val_cells = new_value.CellArray();
				...
				if (lhs_cur.Pointer()->IsCellArray())
				{
					...
					for (int j = 0; j < cells->Size(); j++)
					{
						if (j < val_cells->Size())
							...
							(*cells)(j) = (*val_cells)(val_cells->Size() - 1);
					}
				}
				else
				{
					(*lhs_cur.Pointer()) = (*val_cells)(0); // just take the first one
				}
			}
			else
			{
	...
	if (num_indices) // assignment with indices
	{
		...
		if (target.IsMatrix() || target.IsString() || target.IsPointer())
		{
			...
			else
			{
				...
				if (target.IsLogical() && !value.IsLogical())
					target.SetMask(Currency::MASK_DOUBLE);
		...
		else if (target.IsCellArray())
		{
			if (value.IsCellArray())
			{
				...
				if (indices[0].IsScalar())
				{
					...
					else if ((indices.size() > 1) && (indices[1].IsPositiveIntegralVector()))
					{
						if (value.IsCellArray())
						{
							HML_CELLARRAY* dest_cells = target.CellArray();
							...
							const hwMatrix* indices_mat = indices[1].Matrix();

							if (cells->Size() != indices_mat->Size())
								throw OML_Error(HW_ERROR_INVRHS);
							...
							for (int j = 0; j < indices_mat->Size(); j++)
							{
								int col_index = static_cast<int>((*indices_mat)(j) - 1);

								if (col_index < 0)
									throw OML_Error(OML_ERR_INVALID_INDEX, 2);

								if (col_index >= dest_cells->N())
								{
									if (dest_cells->IsEmpty())
										dest_cells->Dimension(dest_cells->M(), col_index + 1, HML_CELLARRAY::REAL);
									else
										dest_cells->Resize(dest_cells->M(), col_index + 1);
								}

								(*dest_cells)(row_index, col_index) = (*cells)(j);
							}
						}
						else
						{
			...
			else if (value.IsMatrix() && value.Matrix()->Is0x0())
			{
				if (indices.size() == 1)
				{
					Currency idx1 = indices[0];
					...
					else if (idx1.IsMatrix() && idx1.IsEmpty())
					{
						// nothing to do
					}
					else
		...
		{
			if (HasOverloadedFunction(target, "subsasgn"))
			{
				std::vector<Currency> dummy;
				...
				HML_CELLARRAY* cells = new HML_CELLARRAY((int)indices.size(), HML_CELLARRAY::REAL);

				for (int j = 0; j < indices.size(); ++j)
				{
					if (indices[j].IsColon())
						(*cells)(j) = ":";
					else
						(*cells)(j) = indices[j];
				}
				...
				overload.Struct()->IncrRefCount();

				target.ReplaceStruct(overload.Struct());
			}
			...
			{
				throw OML_Error("Undefined operation");
			}
		}
...
void ExprTreeEvaluator::NDAssignmetHelper(Currency& target, const std::vector<Currency>& params, const Currency& value, int refcnt_target)
{
	...
	else if (target.IsScalar())
	{
		std::vector<int> dims;
		dims.push_back(1);
		LHS = allocateMatrixN(dims, true);
		(*LHS)(0) = target.Scalar();
		need_assign = true;
	}
	else if (target.IsCellArray() || target.IsNDCellArray())
	{
		return NDCellAssignmetHelper(target, params, value, refcnt_target);
	}
	else
	{
		throw OML_Error(OML_ERR_ARRAYSIZE);
...
Currency ExprTreeEvaluator::RangeOperator(OMLTree* tree)
{
	...
	if (start.IsObject() || end.IsObject() || incr.IsObject())
	{
		std::vector<Currency> params;
		...
		if (start.IsObject())
		{
			if (HasOverloadedFunction(start, "colon"))
				ret = CallOverloadedFunction("colon", params);
		}
		else if (end.IsObject())
		{
			if (HasOverloadedFunction(end, "colon"))
				ret = CallOverloadedFunction("colon", params);
		}
		else if (incr.IsObject())
		{
			if (HasOverloadedFunction(incr, "colon"))
				ret = CallOverloadedFunction("colon", params);
		}
	...
	if (abort)
		return allocateMatrix(1, 0, true);
	...
	else
	{
		...
		result = allocateMatrix(1, num_steps, true);
	...
	// Need to check if start is an integer or not
	// since this really is only used for indexing
	if (increment == 1.0)
...
Currency ExprTreeEvaluator::TransposeOperator(OMLTree* tree)
{
	...
	if (ret.IsObject())
	{
		return CallOverloadedOperator("transpose", ret);
	}
	else if (ret.IsScalar() || ret.IsComplex())
	{
		return ret;
	}
	else if (ret.IsMatrix())
	{
		...
		hwMatrix* new_mtx = allocateMatrix();
		new_mtx->Transpose(*old_mtx);
		Currency cur(new_mtx);
		cur.SetMask(ret.GetMask());
		return cur;
	}
...
Currency ExprTreeEvaluator::ConjTransposeOperator(OMLTree* tree)
{
	...
	if (ret.IsObject())
	{
		return CallOverloadedOperator("ctranspose", ret);
	}
	...
	{
		StructData* sd = ret.Struct();

		if (sd->GetRefCount() > 1)
			sd = new StructData(*sd);
...
Currency ExprTreeEvaluator::CellValue(OMLTree* tree)
{
	...
	const OMLTree* index_token = tree->GetChild(1);


	if (!target.IsCellArray() && !target.IsNDCellArray() && !target.IsObject())
	{
		if (!_lhs_eval)
	...
	bool old_lhs_eval = _lhs_eval;
	_lhs_eval = false;
	int old_end_context_index = end_context_index;
	...
	end_context_index = old_end_context_index;
...
Currency  ExprTreeEvaluator::CellValueHelper(const Currency& target, const std::vector<Currency>& indices)
{
	...
	if (ret_val.IsMatrix())
		hwMatrix* junk = ret_val.GetWritableMatrix(); // force this to allocate something, otherwise we'll have to check for NULL matrices in a million places // cppcheck-suppress unreadVariable
...
void ExprTreeEvaluator::CellAssignmentHelper(Currency& target, const std::vector<Currency>& params, const Currency& value)
{
	...
	if (num_indices == 2)
	{
		...
		else if (params[0].IsColon() && params[1].IsScalar())
		{
			HML_CELLARRAY* temp = target.CellArray();
			...
			int second_index = (int)params[1].Scalar() - 1;

			if (second_index >= temp->N())
				throw OML_Error(HW_ERROR_INDEXRANGE);

			for (int j = 0; j < temp->M(); ++j)
				(*temp)(j, second_index) = value;
		}
		else if (params[0].IsColon() && params[1].IsPositiveIntegralVector())
		{
			HML_CELLARRAY* temp = target.CellArray();
		...
		else if (params[0].IsScalar() && params[1].IsColon())
		{
			HML_CELLARRAY* temp = target.CellArray();
			...
			int first_index = (int)params[0].Scalar() - 1;

			if (first_index >= temp->M())
				throw OML_Error(HW_ERROR_INDEXRANGE);

			for (int j = 0; j < temp->N(); ++j)
				(*temp)(first_index, j) = value;
		}
		else if (params[0].IsPositiveIntegralVector() && params[1].IsColon())
		{
			HML_CELLARRAY* temp = target.CellArray();
...
Currency ExprTreeEvaluator::InlineIndex(OMLTree* tree)
{
	...
	if (num_func_children == 2) // func + args
	{
		const OMLTree* func_args = tree->GetChild(1);
...
Currency ExprTreeEvaluator::StructValue(OMLTree* tree)
{
	...
	if (struct_tree->GetType() == FUNC)
	{
		...
		{
			if (!ident_tree->u)
				ident_tree->u = (void*)Currency::vm.GetStringPointer(ident_tree->GetText()); // cppcheck-suppress cstyleCast
		...
		if (use_indices)
		{
			const OMLTree* index_tree = struct_tree->GetChild(1);
	...
	if (struct_target.IsStruct() || struct_target.IsObject())
	{
		...
		{
			classname = struct_target.GetClassname();
			ci = (*class_info_map)[classname];
		}
	...
	else if (struct_target.IsPointer())
	{
		...
		if (temp->IsObject())
		{
			...
			if (!ci)
			{
				std::string err_str = "Definition for class " + temp->GetClassname() + " is missing";
				throw OML_Error(err_str);
			}
		}
	...
	if (sd)
	{
		...
		{
			if (!field_tree->u)
				field_tree->u = (void*)Currency::vm.GetStringPointer(field_tree->GetText()); // cppcheck-suppress cstyleCast

			field_name = (const std::string*)field_tree->u; // cppcheck-suppress cstyleCast
		}
		...
		if (ci)
		{
			...
			if (ci->IsPropertyPrivate(*field_name))
			{
				...
				if (!in_class_method)
				{
					if (HasOverloadedFunction(struct_target, "subsref"))
					{
						if (ci->IsSubclassOf("handle"))
						{
							_lhs_eval = true; // cppcheck-suppress redundantAssignment
							...
							struct_target.Pointer()->SetClass(classname);
						}

						std::vector<Currency> dummy;
						dummy.push_back(struct_target);

						StructData* sd = new StructData();
						sd->DimensionNew(1, 1);
						sd->SetValue(0, 0, "type", ".");
						sd->SetValue(0, 0, "subs", *field_name);

						dummy.push_back(sd);

						return CallOverloadedFunction("subsref", dummy);
					}
					else
					{
		...
		if (_lhs_eval)
		{
			...
			Currency* ptr = (Currency*)sd->GetPointer(index_1, index_2, field_name); // cppcheck-suppress cstyleCast
...
Currency ExprTreeEvaluator::StructValueHelper(const Currency* parent, OMLTree* indices, OMLTree* field_tree)
{
	...
	{
		return ObjectMethodCall((Currency*)parent, indices, field_tree); // cppcheck-suppress cstyleCast
	}
	...
	if (!parent->IsStruct() &&!parent->IsObject() && !parent->IsPointer())
	{
		...
		end_context_currency = (Currency*)parent; // cppcheck-suppress cstyleCast
		end_context_varname  = NULL;
	...
	if (num_field_children == 1)
	{
		if (_lhs_eval)
		{
			Currency* ptr = (Currency*)sd->GetPointer(index1, index2, field_name); // cppcheck-suppress cstyleCast
			return Currency(ptr);
		}
	...
	else
	{
		...
		{
			const OMLTree*    index_tree = field_tree->GetChild(1);
...
{  
	...
	const BoundClassInfo* info = _boundclassinfo[in->GetClassname()];
	if (!info)
        return allocateMatrix();
	...
	{
		const OMLTree* args_tree = field->GetChild(1);
...
{
	const ClassInfo* ci = (*class_info_map)[parent->GetClassname()];
	...
	if (field_tree->GetType() == DYN_FIELD)
	{
		...
		bool old_lhs_eval = _lhs_eval;
		_lhs_eval = false;  // cppcheck-suppress redundantAssignment
		Currency temp = RUN(child);
		_lhs_eval = old_lhs_eval;

		if (temp.IsString())
			method_name = temp.StringVal();
	}
	else if (field_tree->GetType() == IDENT)
	{
		method_name = field_tree->GetText();
	}
	...
	else
	{
		...
		{
			if (HasOverloadedFunction(*parent, "subsref"))
			{
				std::vector<Currency> dummy;
				dummy.push_back(*parent);

				StructData* sd = new StructData();
				sd->DimensionNew(1, 1);
				sd->SetValue(0, 0, "type", ".");
				sd->SetValue(0, 0, "subs", method_name);

				dummy.push_back(sd);

				Currency ret = CallOverloadedFunction("subsref", dummy);
				...
				if (ret.IsObject() && num_args)
				{
					std::vector<Currency> local_params;

					local_params.push_back(ret);

					StructData* sd = new StructData();
					sd->DimensionNew(1, 1);
					sd->SetValue(0, 0, "type", "()");

					HML_CELLARRAY* cells = new HML_CELLARRAY(num_args, HML_CELLARRAY::REAL);
					...
					ret = CallOverloadedFunction("subsref", local_params);
				}
				else
				{
					if (num_args)
					{
						// we have to re-run the indices because now we know the context.  Otherwise, end won't work
						inputs.clear(); // we don't want the self object anymore
						
						Currency* old_context_currency = end_context_currency;
						int       old_context_index    = end_context_index;

						end_context_currency = &ret;
						end_context_varname  = NULL;
						end_context_index    = -1;
						...
						for (int j = 0; j < num_args; j++)
						{
							...
							Currency temp = RUN(child_j);
							inputs.push_back(temp);
						}

						end_context_currency = old_context_currency;
						end_context_index    = old_context_index;

						ret = VariableIndex(ret, inputs);
					}
				}
...
{ 
	msm->AddGlobalReference(varname);
	const Currency& ret = msm->GetValue(varname);

	if (ret.IsNothing())
		msm->RemoveGlobalReference(varname);
...
bool ExprTreeEvaluator::ClearFromFunctions(const std::string& name)
{
	...
	{
		UserFunc*     uf = (UserFunc*)func_iter->second;
...
bool ExprTreeEvaluator::RemoveFunctionsInLibrary(const std::string& lib_name)
{
	...
	for (iter = std_functions->begin(); iter != std_functions->end(); iter++)
	{
		...
		if (dot_index != std::string::npos)
			dllname = dllname.substr(0, dot_index); // cppcheck-suppress  uselessCallsSubstr

		if (dot_index2 != std::string::npos)
			dllname2 = dllname2.substr(0, dot_index2); // cppcheck-suppress  uselessCallsSubstr
...
bool ExprTreeEvaluator::ClearFromVariablesExcept(const std::vector<std::string>& varnames,
												const std::vector<std::regex>& varwildnames,
												const std::vector<std::string>& exceptnames,
												const std::vector<std::regex>& exceptwildnames)
{
	return msm->RemoveVariablesExcept(varnames, varwildnames, exceptnames, exceptwildnames);
}

bool ExprTreeEvaluator::ClearFromGlobalsExcept(const std::vector<std::string>& varnames,
	const std::vector<std::regex>& varwildnames,
	const std::vector<std::string>& exceptnames,
	const std::vector<std::regex>& exceptwildnames)
{
	return msm->RemoveGlobalsExcept(varnames, varwildnames, exceptnames, exceptwildnames);
}
...
void ExprTreeEvaluator::ClearFunctions()
{
    ...
    for (iter = functions->begin(); iter != functions->end();)
    {
		...
		if (!locked)
		{
			UserFunc* uf = (UserFunc*)iter->second; // cppcheck-suppress cstyleCast
			FunctionInfo* fi = uf->fi;
...
void ExprTreeEvaluator::ClearClassesAndObjects()
{
	// also need to clear out any variables of type OBJECT somehow

	class_info_map->clear();

	// clear out constructors from the function table
	std::vector<std::string> to_clear;

	std::map<std::string, UserFunc*>::iterator iter;
	for (iter = functions->begin(); iter != functions->end(); ++iter)
	{
		if (iter->second->fi->IsConstructor())
			to_clear.push_back(iter->first);
	}
...
int ExprTreeEvaluator::GetContextEndValue()
{
	Currency context;
	...
	if (context.IsMatrixOrString())
	{
		...
		else
		{
			if (end_context_index == -1)
			{
				return (int)utf8_strlen((unsigned char*)context.StringVal().c_str()); // cppcheck-suppress cstyleCast
			}
			else if (end_context_index == 0)
			{
				return mtx->M();
	...
	{
		std::vector<Currency> dummy;
		dummy.push_back(context);

		if (end_context_index == -1)
		{
			if (HasOverloadedFunction(context, "size"))
			{
				Currency ret = CallOverloadedFunction("size", dummy);
				const hwMatrix* mat = ret.Matrix();
			...
			{
				const StructData* sd = context.Struct();
				return (sd->M() * sd->N());
			}
		}
		else
		{
			if (HasOverloadedFunction(context, "size"))
			{
				Currency ret = CallOverloadedFunction("size", dummy);
				const hwMatrix* mat = ret.Matrix();
				return int((*mat)(end_context_index)); // cppcheck-suppress cstyleCast
			}
			else
			{
				const StructData* sd = context.Struct();

				if (end_context_index == 0)
					return sd->M();
				else if (end_context_index == 1)
					return sd->N();
				else
...
Currency ExprTreeEvaluator::ExecuteCatch(const OML_Error& error, OMLTree* tree)
{
	...
	std::string msg(FormatErrorMessage(error.GetErrorMessage())); // This would have the line number
	...
	if (num_children > 1)
	{
		for (int j = num_children - 1; j > 0; j--)
		{
			...
			if (child->GetType() == IDENT)
			{
				...
				StructData* sd = new StructData();
				sd->DimensionNew(1, 1);
				sd->SetValue(0, 0, "message", error.GetErrorMessage());
...
Currency ExprTreeEvaluator::TryCatch(OMLTree* tree)
{
	...
	{
		return ExecuteCatch(error, tree);
	}
	catch (hwMathException& e)
	{
		return ExecuteCatch(OML_Error(e.what()), tree);
	}
	catch (std::bad_alloc& e)
	{
		return ExecuteCatch(OML_Error(HW_MATH_ERR_ALLOCFAILED), tree);
	}
...
Currency ExprTreeEvaluator::InPlaceExpansion(OMLTree* tree)
{
	...
	if (!var_tree->u)
		var_tree->u = (void*)Currency::vm.GetStringPointer(var_tree->GetText()); // cppcheck-suppress cstyleCast

	std::string* pString = (std::string*)var_tree->u; // cppcheck-suppress cstyleCast

	if (!msm->IsGlobal(*pString) && !msm->Contains(pString) && !msm->IsPersistent(pString))
	{
		char buffer[2048];
		sprintf(buffer, "Error: '%s' is not defined", pString->c_str());
				...
				if (added_cells->M() == cells->M())
				{
					...
					for (int k=0; k<cells->M(); k++)
					{
						for (int j=0; j<added_cells->N(); j++)
							(*cells)(k, start_col+j) = (*added_cells)(k, j);
					}

					return original;
				}
				else if (added_cells->IsEmpty())
				{
					return original;
				}
			}
...
Currency ExprTreeEvaluator::ClassDefinition(OMLTree* tree)
{
			...
			for (int j=0; j<num_methods; j++)
			{
				...
				if (fi)
				{
					if (fi->FunctionName() == *class_name) // it's the constructor
					{
						...
						ci->SetConstructorFunctionInfo(fi);
	...
	{
		OMLTree* subclass_tree = NULL;

		for (int j = 0; j < num_sections; j++)
		{
			OMLTree* test_tree = tree->GetChild(j);

			if (test_tree->GetType() == ID_LIST)
			{
				subclass_tree = test_tree;
				break;
			}
		}
...
bool ExprTreeEvaluator::ParseAndRunFile(const std::string& file_name, bool allow_script)
{
	...
	if (!input)
	{
		std::string error_message = "File " + file_name;
		error_message += " is not accessible";
	...
	while (keep_going)
	{
		...
		{
			char buffer[8];
			sprintf(buffer, "%d", parser->pParser->rec->state->exception->line);

			std::string error_message = "Syntax error at line number ";
			error_message += buffer;
			error_message += " in included file " + file_name;
	...
	if (!ValidateFunction(root_tree))
	{
		if (!allow_script)
		{
			std::string error_message = "OML function file " + file_name + " cannot have executable statements";
			nested_function_marker = temp;
			throw OML_Error(error_message);
		}
		else
...
bool  ExprTreeEvaluator::RegisterBuiltInFunction(const std::string& func_name, FUNCPTR fp, int nargin, int nargout, bool thread_safe)
{
	(*std_functions)[func_name] = BuiltinFunc(fp, FunctionMetaData(nargin, nargout), _dll_context, thread_safe);
	OnUpdateFuncList();
	return true;
}

bool  ExprTreeEvaluator::RegisterBuiltInFunction(const std::string& func_name, FUNCPTR fp, FunctionMetaData fmd, bool thread_safe)
{
	(*std_functions)[func_name] = BuiltinFunc(fp, fmd, _dll_context, thread_safe);
	OnUpdateFuncList();
	return true;
}

bool  ExprTreeEvaluator::RegisterBuiltInFunction(const std::string& func_name, ALT_FUNCPTR fp, bool thread_safe)
{
	(*std_functions)[func_name] = BuiltinFunc(fp, _dll_context, _dll_user_hierarchy, thread_safe);
	OnUpdateFuncList();
	return true;
}

bool  ExprTreeEvaluator::RegisterBuiltInFunction(const std::string& func_name, ALT_FUNCPTR fp, FunctionMetaData fmd, bool thread_safe)
{
	(*std_functions)[func_name] = BuiltinFunc(fp, fmd, _dll_context, _dll_user_hierarchy, thread_safe);
	OnUpdateFuncList();
	return true;
}
...
void ExprTreeEvaluator::PreprocessAST(pANTLR3_BASE_TREE tree, pANTLR3_COMMON_TOKEN_STREAM tokens)
{
	...
	if(tok) 
	{
		...
		else if (tok->type == FUNC)
		{
			...
			if ((first_token->type == CLEAR) || (first_token->type == CLEARVARS))
				first_token->type = IDENT;
		}
...
hwMatrix* ExprTreeEvaluator::SubmatrixSingleIndexHelper(Currency& target, const Currency& indices, const Currency& value, int target_refcnt)
{
	...
	else if (indices.GetMask() == Currency::MASK_DOUBLE)
	{
		if (value.IsEmpty())
		{
			...
			{
				int erase_idx = to_delete[j];

				if (erase_idx > to_keep.size())
					throw OML_Error(HW_ERROR_INVIND);

				to_keep.erase(to_keep.begin() + erase_idx-1, to_keep.begin() + erase_idx);
			}

			int result_size = (int)to_keep.size();

			if (data->M() == 1)
				new_matrix = allocateMatrix(1, result_size, data->IsReal());
			else
				new_matrix = allocateMatrix(result_size, 1, data->IsReal());

			if (new_matrix->IsReal())
			{
				for (int j = 0; j < to_keep.size(); ++j)
					(*new_matrix)(j) = (*data)(to_keep[j] - 1);
			}
			else
			{
				for (int j = 0; j < to_keep.size(); ++j)
					new_matrix->z(j) = data->z(to_keep[j] - 1);
			}
		...
		else
		{
			...
			for (int j=0; j<index->Size(); j++)
			{
				...
				else if (value.IsMatrix() || value.IsString())
				{
					...
					bool implicit_transpose = false;
					if ((val_mtx->M() == index->N()) && (val_mtx->N() == 1) && (index->M() == 1))
						implicit_transpose = true;
					...
					if (!implicit_transpose)
					{
						...
						if (val_mtx->N() != index->N())
						{
							if (val_mtx->N() == 1)
							{
								tmp_mtx = allocateMatrix(val_mtx->M(), index->N(), true); // complex case handled above
		...
		else if (value.IsMatrix() || value.IsString())
		{
			...
			if ((values->M() == 0) && (values->N() == 0))
			{
				...
				if (new_matrix->IsReal())
				{
					if (new_matrix->Size())
					{
						hwMatrix* result = allocateMatrix(num_rows, num_cols, true);
				...
				{
					hwMatrix* result = allocateMatrix(num_rows, num_cols, false);
		...
		else if (index2.IsVector())
		{
			...
			else if (value.IsMatrix())
			{
				...
				if (index2.IsLogical())
				{
					...
					try
					{
						oml_find(ei, inputs, outputs);
					}
					catch (OML_Error)
					{
						// do nothing and try it the regular way below
					}
					...
					if (outputs.size() == 1)
					{
						Currency new_indices = outputs[0];

						if (new_indices.IsVector())
							indices = new_indices.Vector();
					}
				}
				...
				if (index2.IsLinearRange())
				{
					new_matrix->WriteSubmatrix(0, (int)indices[0] - 1, *val_matrix);
				}
				else
				{
					for (int j = 0; j < indices.size(); j++)
					{
						...
						hwMatrix col_matrix;
						val_matrix->ReadColumn(j, col_matrix);
						new_matrix->WriteColumn(index2 - 1, col_matrix);
					}
				}
			}
		}
		...
		else if (index1.IsVector())
		{
				...
				for (int j=0; j<indices.size(); j++)
				{
					...
					for (int k = 0; k < new_matrix->N(); k++)
					{
						if (new_matrix->IsReal())
							...
							new_matrix->z((int)indices[j] - 1, k) = value.Scalar();
					}
				}
			}
			else if (value.IsComplex())
			...
			else if (value.IsEmpty())
			{
				if (index1.IsLogical())
				{
					...
					{
						if (indices[j] != 0.0)
							hwMathStatus stat = new_matrix->DeleteRows(j, 1);
					}
				}
				else
				{
			...
			else if (value.IsMatrix())
			{
				...
				if (index1.IsLinearRange())
				{
					new_matrix->WriteSubmatrix((int)indices[0] - 1, 0, *val_matrix);
				}
				else
				{
					for (int j = 0; j < indices.size(); j++)
					{
						int idx1 = (int)indices[j];

						hwMatrix row_matrix;
						val_matrix->ReadRow(j, row_matrix);
						new_matrix->WriteRow(idx1 - 1, row_matrix);
					}
				}
		    }
		}
	...
	else
	{
		if (value.IsMatrixOrString() || value.IsScalar() || value.IsComplex())
		{
			...
			else if (index1.IsPositiveIntegralVector() && index2.IsPositiveIntegralVector())
			{
				...
				if (is_real)
				{
					if (index1.IsLinearRange() && index2.IsLinearRange() && val_mtx)
					{
						int row_offset = (int)indices1[0]-1;
						int col_offset = (int)indices2[0]-1;

						new_matrix->WriteSubmatrix(row_offset, col_offset, *val_mtx);
					}
					else
					{
						double new_val;
				...
				{
					if (index1.IsLinearRange() && index2.IsLinearRange())
					{
						int row_offset = (int)indices1[0] - 1;
						int col_offset = (int)indices2[0] - 1;
...
void ExprTreeEvaluator::PushResult(const Currency& cur, bool to_output)
{
	...
	{
        if (BuiltInFuncsUtils::IsOutputLogOpen())
        {
            const_cast<Currency&>(cur).SetOutputTypeToLog();
        }
...
void ExprTreeEvaluator::PrintResult(Currency cur)
{
    ...
    if (BuiltInFuncsUtils::IsOutputLogOpen())
    {
        static_cast<Currency*>(&cur)->SetOutputTypeToLog();
    }
...
bool ExprTreeEvaluator::RootPathMatches(const std::string& s1, const std::string& s2) const
{
	// s1 is the rootpath
	// s2 is the path
#ifdef OS_WIN
	...
	std::replace(ls2.begin(), ls2.end(), '/', '\\');
	// return ls1 == ls2;
	// return ls2.starts_with(ls1);
	std::string leadstr = ls2.substr(0, ls1.length());
	return ls1 == leadstr;
#else
	// return s1 == s2;
	// return s2.starts_with(s1);
	std::string leadstr = s2.substr(0, s1.length());
	return s1 == leadstr;
#endif
}

void ExprTreeEvaluator::AddPath(const std::string& pathname, bool end)
{
	// avoid repeats
...
void ExprTreeEvaluator::UpdatePathCache(const std::string& pathname)
{
	const std::string* path = Currency::pm.GetStringPointer(pathname);

	std::string search_item = pathname + "/*.om*";
	std::vector<std::string> contents = BuiltInFuncsUtils::GetMatchingFiles(search_item);

	std::string secondary_search_item = pathname + "/*.m*";
	std::vector<std::string> secondary_contents = BuiltInFuncsUtils::GetMatchingFiles(secondary_search_item);

	std::vector<std::string>::const_iterator iter;

	for (iter = secondary_contents.begin(); iter != secondary_contents.end(); ++iter)
		contents.push_back(*iter);

	(*cached_path_contents)[path] = contents;
}

void ExprTreeEvaluator::RefreshPathCache()
{
	std::map < const std::string*, std::vector<std::string>>::iterator iter;

	for (iter = cached_path_contents->begin(); iter != cached_path_contents->end(); ++iter)
		UpdatePathCache(*(iter->first));
}


void ExprTreeEvaluator::AddHiddenPath(const std::string& pathname)
{
	// avoid repeats
	if (!IsInPaths(pathname))
	{
...
bool ExprTreeEvaluator::RemovePath(const std::string &pathname)
{
	std::vector<std::string>::iterator iter;
...
bool ExprTreeEvaluator::RemoveHiddenPath(const std::string& pathname)
{
	std::vector<std::string>::iterator iter;
	for (iter = hidden_paths->begin() + NUM_MANDATORY_PATHS; iter != hidden_paths->end(); iter++)
	{
		if (PathMatches(pathname, *iter))
		{
			hidden_paths->erase(iter);
			RemovePath(pathname);
...
bool ExprTreeEvaluator::RemoveHiddenRootPath(const std::string& pathname)
{
	std::vector<std::string>::iterator iter;
	std::vector<std::string> hidden_paths_retained;
	bool result = false;  // default 

	for (iter = hidden_paths->begin() + NUM_MANDATORY_PATHS; iter != hidden_paths->end(); iter++)
	{
		std::string test = *iter;
		if (RootPathMatches(pathname, *iter))	// path begins with the root pathname
		{
			// DEBUG_PRINT("Remove Hidden Path: " + test)
			RemovePath(*iter);
			result = true; // when a path has been deleted.
		}
		else
		{
			hidden_paths_retained.push_back(*iter);
		}
	}
	if (result)
	{
		hidden_paths->clear();
		for (int j = 0; j < hidden_paths_retained.size(); ++j)
		{
			hidden_paths->push_back((hidden_paths_retained)[j]);
		}
	}
	return result;
}

bool ExprTreeEvaluator::FindFileInPath(const std::string& file_plus_ext, std::string& filepath)
{
	std::string cur_path = BuiltInFuncsUtils::GetCurrentWorkingDir();
	std::replace(cur_path.begin(), cur_path.end(), '\\', '/'); // slash direction is important for the debugger
	paths->insert(paths->begin(), cur_path);
	...
	if (file_plus_ext.find_first_of("/\\") != std::string::npos)
		file_has_slash = true;
...
{
	paths->clear();
	// VSM-5385 Restore multiple directory default path
	for (int j = 0; j < restore_paths->size(); ++j)
	{
		paths->push_back((*restore_paths)[j]);
	}

	// VSM-5385 This includes hidden paths
	hidden_paths->clear();
	for (int j = 0; j < restore_hidden_paths->size(); ++j)
	{
		hidden_paths->push_back((*restore_hidden_paths)[j]);
	}
}
...
MemoryScope* ExprTreeEvaluator::GetValidErrorScope()
{
	// split the folder and function names from error_file
// check treat_as_builtin_list for that folder
// if found, try the parent scope - repeat until find one that isn't in the list
// use that scope for the error_file and error_line
// set builtin_error_scope to the function name from step 1
	MemoryScope* scope = NULL;

	for (int j = 0; j < msm->GetStackDepth(); ++j)
	{
		scope = msm->GetScope(j);
		bool found_it = false;

		if (scope)
		{
			std::string  full_path = scope->GetFilename();
			...
			{
				std::string folder = full_path.substr(0, slash_pos);

				std::vector<std::string>::iterator iter;
				for (iter = treat_as_builtin_list->begin(); iter != treat_as_builtin_list->end(); iter++)
				{
					if (*iter == folder)
					{
						found_it = true;
						builtin_error_scope = scope->FunctionName();
						break;
					}
				}
			}
...
{
	MemoryScope* scope = GetValidErrorScope();
...
hwMatrix* ExprTreeEvaluator::allocateMatrix(int m, int n, void* data, bool isReal)
{
    CheckDims(m,n);
	hwMatrix* mtx = new hwMatrix(m, n, data, isReal ? hwMatrix::REAL : hwMatrix::COMPLEX);
    CheckSize(mtx, m*n);
	return mtx;
}

hwMatrix* ExprTreeEvaluator::allocateMatrix(int m, int n, bool isReal)
{
    CheckDims(m,n);
	hwMatrix* mtx = new hwMatrix(m, n, isReal ? hwMatrix::REAL : hwMatrix::COMPLEX);
    CheckSize(mtx, m*n);
	return mtx;
}
...
{
    CheckDims(m,n);
	hwMatrix* mtx = ExprTreeEvaluator::allocateMatrix(m, n, true);
    CheckSize(mtx, m*n);
	mtx->SetElements(value);
	return mtx;
}

hwMatrix* ExprTreeEvaluator::allocateMatrix(int m, int n, const hwComplex& value)
{
    CheckDims(m,n);
	hwMatrix* mtx = ExprTreeEvaluator::allocateMatrix(m, n, false);
    CheckSize(mtx, m*n);
	mtx->SetElements(value);
	return mtx;
}

hwMatrixI* ExprTreeEvaluator::allocateMatrixI(int m, int n, bool isReal)
{
    CheckDims(m,n);
	hwMatrixI* mtx = new hwMatrixI(m, n, isReal ? hwMatrixI::REAL : hwMatrixI::COMPLEX);
    CheckSize(mtx, m*n);
	return mtx;
}

hwMatrixI* ExprTreeEvaluator::allocateMatrixI(int m, int n, int val)
{
    CheckDims(m,n);
	hwMatrixI* mtx = ExprTreeEvaluator::allocateMatrixI(m, n, true);
    CheckSize(mtx, m*n);
	mtx->SetElements(val);
	return mtx;
}
...
hwMatrixN* ExprTreeEvaluator::allocateMatrixN(const std::vector<int>& dims, bool isReal)
{
	return new hwMatrixN(dims, isReal ? hwMatrixN::REAL : hwMatrixN::COMPLEX);
}
...
FunctionInfo* ExprTreeEvaluator::FunctionInfoFromString(const std::string& str)
{
	...
	if (parser->pParser->rec->getNumberOfSyntaxErrors(parser->pParser->rec) > 0)
	{
		throw OML_Error("Invalid string");
	}
...
bool ExprTreeEvaluator::HasOverloadedFunction(const Currency& obj, const std::string& func_name)
{
	...
	else if (obj.IsPointer() && obj.Pointer()->IsObject())
	{
		std::string class_name = obj.Pointer()->GetClassname();
		ClassInfo* ci = (*class_info_map)[class_name];
...
Currency ExprTreeEvaluator::CallOverloadedFunction(const std::string& func_name, const std::vector<Currency>& inputs)
{
	...
	{
		if (inputs[j].IsObject())
		{
			class_name = inputs[j].GetClassname();
			ci = (*class_info_map)[class_name];
			break;
		}
		else if (inputs[j].IsPointer())
		{
			const Currency* my_pointer = inputs[j].Pointer();

			if (my_pointer->IsObject())
			{
				class_name = inputs[j].Pointer()->GetClassname();
				ci = (*class_info_map)[class_name];
				break;
			}
		}
	...
	if (ci)
		fi = ci->GetFunctionInfo(func_name);
...
void ExprTreeEvaluator::RunEncryptedFile(const std::string& extension, const std::string& file_name)
{
	...
	if (decrypt)
	{
		...
		if (!decrypted)
			throw OML_Error("Invalid encrypted file");
...
void ExprTreeEvaluator::RunFile(const std::string& filename)
{
	...
	if (!input) // should only happen with an empty file so there's nothing to do
		return;
...
void ExprTreeEvaluator::SetApplicationDir(const std::string& dir)
{
	...
	{
		restore_paths->push_back((*paths)[j]);
	}

	// VSM-5385
	restore_hidden_paths->clear();
	for (int j = 0; j < hidden_paths->size(); ++j)
	{
		restore_hidden_paths->push_back((*hidden_paths)[j]);
	}
}
...
void AnalyzeHelper(OMLTree* tree, std::set<std::string>& required, std::set<std::string>& defined)
{
	...
	else if (type == FUNC)
	{
		...
		// cycle through all the arguments here too
		if (tree->ChildCount() == 2)
		{
			OMLTree* param_tree = tree->GetChild(1);

			for (int j = 0; j < param_tree->ChildCount(); ++j)
				AnalyzeHelper(param_tree->GetChild(j), required, defined);
		}
	}
	else if (type == IDENT)
	{
	...
	else if (type == FUNC_HANDLE)
	{
		...
		if (tree->ChildCount() == 4)
			AnalyzeHelper(tree->GetChild(3), required, defined);
	}
	else if (type == GLOBAL)
	{
		for (int j=0; j<tree->ChildCount(); j++)
...
Currency ExprTreeEvaluator::GetMetadata(const std::string& infile, bool library_names)
{
	pANTLR3_INPUT_STREAM input = ANTLRData::InputFromFilename(infile);
	...
	std::vector<std::string> needed;
	std::vector<std::string> modules_needed;
		...
		if (defined.find(test) == defined.end())
		{
			...
			if (fptr)
			{
				...
				else
				{
					...
					if (std::find(modules_needed.begin(), modules_needed.end(), module) == modules_needed.end())
						modules_needed.push_back(module);
				}
			}
		}
	}
	...
	{
		HML_CELLARRAY* ans_cell = ExprTreeEvaluator::allocateCellArray(1, (int)modules_needed.size());

		for (int j = 0; j < modules_needed.size(); j++)
			(*ans_cell)(j) = modules_needed[j];
...
//------------------------------------------------------------------------------
bool ExprTreeEvaluator::LockBuiltInFunction(const std::string& funcname, bool hide)
{
...
Currency ExprTreeEvaluator::GetProfileData() const
{
	int num_files = (int)profile_data.size();

	HML_CELLARRAY* cells = new HML_CELLARRAY(num_files, 2, HML_CELLARRAY::REAL);

	std::map<const std::string*, std::map<int, std::tuple<double, int>>>::const_iterator iter;

	int outer_counter = 0;

	for (iter = profile_data.begin(); iter != profile_data.end(); ++iter)
	{
		(*cells)(outer_counter, 0) = *iter->first;

		const std::map<int, std::tuple<double, int>>& temp_map = iter->second;
		std::map<int, std::tuple<double, int>>::const_iterator temp_iter;

		int num_lines = (int)temp_map.size();

		hwMatrix* data = new hwMatrix(num_lines, 3, hwMatrix::REAL);

		int inner_counter = 0;

		for (temp_iter = temp_map.begin(); temp_iter != temp_map.end(); ++temp_iter)
		{
			(*data)(inner_counter, 0) = temp_iter->first;
			(*data)(inner_counter, 1) = std::get<0>(temp_iter->second);
			(*data)(inner_counter, 2) = std::get<1>(temp_iter->second);
			++inner_counter;
		}
		
		(*cells)(outer_counter, 1) = data;
		++outer_counter;
	}
...
void ExprTreeEvaluator::ClearProfileData()
{
	profile_data.clear();
}

void ExprTreeEvaluator::DisableWarning(const std::string& id)
{
	disabled_warnings.push_back(id);
}

void ExprTreeEvaluator::EnableWarning(const std::string& id)
{
	std::vector<std::string>::iterator iter;

	iter = std::find(disabled_warnings.begin(), disabled_warnings.end(), id);

	if (iter != disabled_warnings.end())
		disabled_warnings.erase(iter);
}

bool ExprTreeEvaluator::IsWarningDisabled(const std::string& id)
{
	std::vector<std::string>::iterator iter;
...
void ExprTreeEvaluator::TreatAsBuiltin(const std::string& path)
{
	std::string my_path = path;
	std::replace(my_path.begin(), my_path.end(), '\\', '/');
	treat_as_builtin_list->push_back(my_path);
}

// ProfileTimer stuff
ProfileTimer* ProfileTimer::_timers = NULL;
int ProfileTimer::_counter = -1;

void ProfileTimer::Initialize()
{
	if (!_timers)
	{
		_timers = new ProfileTimer[1024];
		_counter = 0;
	}
}

ProfileTimer* ProfileTimer::GetNext()
{
	 ProfileTimer* ret = &(_timers[_counter]);
	_counter++;
	return ret;
}

void ProfileTimer::ReleaseLast()
{
	_counter--;
	_timers[_counter].Reset();
}

void ExprTreeEvaluator::RegisterLibraryAlias(const std::string& path, const std::string& alias)
{
	(*_library_aliases)[path] = alias;
}

std::string ExprTreeEvaluator::GetLibraryAlias(const std::string& path) const
{
	std::map<std::string, std::string>::const_iterator iter;

	iter = _library_aliases->find(path);

	if (iter == _library_aliases->end())
		return "";
	else
		return iter->second;
}

ClassInfo* ExprTreeEvaluator::GetClassInfoFromName(const std::string& classname) const
{
	ClassInfo* ci = NULL;

	if (class_info_map->find(classname) != class_info_map->end())
		ci = (*class_info_map)[classname];
		
	return ci;
}

std::string ExprTreeEvaluator::GetFunctionArgumentName(int index)
{
	std::string ret_val;

	if (function_args.size() > 1)
	{
		OMLTree* arg_tree = function_args[function_args.size() - 2]; // -2 because the last one is the arg for inputname

		if (arg_tree->ChildCount() >= index)
		{
			OMLTree* child = arg_tree->GetChild(index-1);

			if (child->GetType() == IDENT)
				ret_val = child->GetText();
		}
	}
...
void ExprTreeEvaluator::RemoveChildEvaluator()
{
	if (!child_interps.empty())
	{
		child_interps.pop_back();
	}
}

void ExprTreeEvaluator::CacheBCIPointer(OMLImplBase* in_ptr)
{
	bci_cached_pointers.push_back(in_ptr);
}

void ExprTreeEvaluator::BCIGarbageCollect()
{
	for (int j = 0; j < bci_cached_pointers.size(); j++)
		delete bci_cached_pointers[j];

	bci_cached_pointers.clear();
}

OMLTree* ExprTreeEvaluator::CreateASTFromFile(const std::string& filename)
{
	pANTLR3_INPUT_STREAM input = ANTLRData::InputFromFilename(filename);
	ANTLRData ad(input, true);
...
typedef OMLAST* (*TestFP)(OMLInterface* eval, OMLCurrencyList4* inputs);

OMLTree* ExprTreeEvaluator::GetTreeFromDLL(const char* dll_path)
{
	void* vResult = BuiltInFuncsCore::DyLoadLibrary(dll_path);

	if (!vResult)
		throw OML_Error(OML_ERR_INVALID_DLL);

	void* symbol = BuiltInFuncsCore::DyGetFunction(vResult, "GetGeneratedTree");
	if (!symbol)
		throw OML_Error(OML_ERR_INVALID_DLL);

	TestFP fp = (TestFP)symbol;
	
	EvaluatorInterface ei(this);

	OMLInterfaceImpl* impl = new OMLInterfaceImpl(&ei);
	OMLCurrencyListImpl* in_list = new OMLCurrencyListImpl(&ei);

	OMLAST* foo = fp(impl, in_list);
	OMLASTImpl* tree_impl = (OMLASTImpl*)foo;

	return tree_impl->GetTree();
}