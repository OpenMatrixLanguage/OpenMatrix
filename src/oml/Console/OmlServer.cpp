...
public:
    ...
    Internal(Interpreter* interp, OmlServer* omlserver):
      _sock(INVALID_SOCKET), _interp(interp), _omlServer(omlserver)
    {
        InitMutex();
    }
    //! Destructor
    ...
    bool Start(const std::string& port)
    {
        ...
        std::unique_ptr<ServerWrapper> Wrapper(new ServerWrapper(_interp, _omlServer));
...
//! Constructor
//! \param[in] interpreter
OmlServer::OmlServer(Interpreter* interp)
{
    _internal = new Internal(interp, this);
}
//! Destructor
OmlServer::~OmlServer()
{
