...
// Constructor
//------------------------------------------------------------------------------
hwHamming::hwHamming(bool periodic)
...
// Constructor
//------------------------------------------------------------------------------
hwHamming::hwHamming(int  numPoints,
                     bool periodic)
...
//------------------------------------------------------------------------------
hwMathStatus hwHamming::ComputeWeights(hwMatrix& weight)
{
    if (!weight.IsReal())
    {
    ...
    if (m_windowType == Periodic)
    {
        ...
        {
            weight(j) = 0.54 - 0.46 * cos((2*PI*j) / size);
        }
        return hwMathStatus();
    }
    
    // m_windowType is Symmetric
    if (size == 1)
    {
        weight(0) = 1;
        return hwMathStatus();
    ...
    for (int j = 0; j < size; ++j)
    {
        weight(j) = 0.54 - 0.46 * cos((2*PI*j) / (size - 1.0));
    }
