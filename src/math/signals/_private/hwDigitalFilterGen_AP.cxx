/**
* @file  hwDigitalFilterGen_AP.cxx
* @date April 2009
...
//------------------------------------------------------------------------------
// Constructor: Base class for digital filters with only poles in the S plane
//! Base class for digital filters with only poles in the S plane
//------------------------------------------------------------------------------
hwDigitalFilterGen_AP::hwDigitalFilterGen_AP(const hwFilterSpecs& filterSpecs,
...
// Destructor
//------------------------------------------------------------------------------
hwDigitalFilterGen_AP::~hwDigitalFilterGen_AP()
{
}
//------------------------------------------------------------------------------
// Compute transfer function polynomial coefficients for a low pass filter
//------------------------------------------------------------------------------
void hwDigitalFilterGen_AP::CreateLowPassFilter()
{
    ...
    else
    {
        ...
        // first pole pair
        m_pDigitalFilter->GetSPlaneInfo(0, omegaC, omegaCSq, sPlanePoleReal, sPlanePoleMagSq);
        numerCoef[0] = sPlanePoleMagSq;
        numerCoef[1] = 2.0 * sPlanePoleMagSq;
        numerCoef[2] = sPlanePoleMagSq;
        ...
        {
            m_pDigitalFilter->GetSPlaneInfo(i, omegaC, omegaCSq, sPlanePoleReal, sPlanePoleMagSq);
            nfc[0] = sPlanePoleMagSq;
            nfc[1] = 2.0 * sPlanePoleMagSq;
            nfc[2] = sPlanePoleMagSq;
...
//------------------------------------------------------------------------------
void hwDigitalFilterGen_AP::CreateHighPassFilter()
{
    // coefficients are stored in descending powers of z
    ...
    else
    {
        ...
        int k = 0;
        double nfc[3];          // numerator factor coefficients for a pole pair
        double dfc[3];          // denominator factor coefficients

        for (int i = 1; i < m_pFilterSpecs->Order()/2; ++i)
        {
            m_pDigitalFilter->GetSPlaneInfo(i, omegaC, omegaCSq, sPlanePoleReal,
                                            sPlanePoleMagSq);
            nfc[0] = sPlanePoleMagSq;
            nfc[1] = -2.0 * sPlanePoleMagSq;
            nfc[2] = sPlanePoleMagSq;
...
//------------------------------------------------------------------------------
void hwDigitalFilterGen_AP::CreateBandPassFilter()
{
    // coefficients are stored in descending powers of z
    ...
    else
    {
        ...
        numerCoef[0] =  sPlanePoleMagSq;
        numerCoef[1] =  0.0;
        numerCoef[2] = -2.0 * sPlanePoleMagSq;
        numerCoef[3] =  0.0;
        numerCoef[4] =  sPlanePoleMagSq;
        ...
        for (int i = 1; i < m_pFilterSpecs->Order()/2; ++i)
        {
            ...
            nfc[0] =  sPlanePoleMagSq;
            nfc[1] =  0.0;
            nfc[2] = -2.0 * sPlanePoleMagSq;
            nfc[3] =  0.0;
            nfc[4] =  sPlanePoleMagSq;
...
//------------------------------------------------------------------------------
void hwDigitalFilterGen_AP::CreateBandStopFilter()
{
    // coefficients are stored in descending powers of z
    double bandWidth = m_pFilterSpecs->GetUpperCorner() - m_pFilterSpecs->GetLowerCorner();
    double bandCenter = 0.5 * (m_pFilterSpecs->GetLowerCorner() + m_pFilterSpecs->GetUpperCorner());
    ...
    else
    {
        ...
        m_pDigitalFilter->GetSPlaneInfo(0, omegaC, omegaCSq, sPlanePoleReal, sPlanePoleMagSq);
        numerCoef[0] = sPlanePoleMagSq;
        numerCoef[1] = -4.0 * alpha * sPlanePoleMagSq;
        numerCoef[2] = (2.0 + 4.0 * alpha * alpha) * sPlanePoleMagSq;
        ...
        for (int i = 1; i < m_pFilterSpecs->Order()/2; ++i)
        {
            m_pDigitalFilter->GetSPlaneInfo(i, omegaC, omegaCSq, sPlanePoleReal, sPlanePoleMagSq);
            nfc[0] = sPlanePoleMagSq;
            nfc[1] = -4.0 * alpha * sPlanePoleMagSq;
            nfc[2] = (2.0 + 4.0 * alpha * alpha) * sPlanePoleMagSq;
            nfc[3] = nfc[1];
            nfc[4] = sPlanePoleMagSq;
            dfc[0] = sPlanePoleMagSq - 2.0 * sPlanePoleReal + 1.0;
