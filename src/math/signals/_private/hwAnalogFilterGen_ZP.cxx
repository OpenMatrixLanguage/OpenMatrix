...
// Constructor
//------------------------------------------------------------------------------
hwAnalogFilterGen_ZP::hwAnalogFilterGen_ZP(const hwFilterSpecs& filterSpecs,
...
// Destructor
//------------------------------------------------------------------------------
hwAnalogFilterGen_ZP::~hwAnalogFilterGen_ZP()
{
}
//------------------------------------------------------------------------------
// Compute transfer function polynomial coefficients for a low pass filter
//------------------------------------------------------------------------------
void hwAnalogFilterGen_ZP::CreateLowPassFilter()
{
    ...
    else
    {
        ...
        m_pAnalogFilter->GetSPlaneInfo(0, sPlanePoleReal, sPlanePoleMagSq, sPlaneZeroMagSq);
        numerCoef[0] = sPlanePoleMagSq / sPlaneZeroMagSq;
        numerCoef[1] = 0.0;
        ...
        {
            m_pAnalogFilter->GetSPlaneInfo(i, sPlanePoleReal, sPlanePoleMagSq, sPlaneZeroMagSq);
            nfc[0] = sPlanePoleMagSq / sPlaneZeroMagSq;
            nfc[1] = 0.0;
...
//------------------------------------------------------------------------------
void hwAnalogFilterGen_ZP::CreateHighPassFilter()
{
    // coefficients are stored in descending powers of s
    ...
    else
    {
        ...
        numerCoef[1] = 0.0;
        numerCoef[2] = omegaCSq * sPlanePoleMagSq / sPlaneZeroMagSq;
        ...
        for (int i = 1; i < m_pFilterSpecs->Order()/2; ++i)
        {
            ...
            nfc[1] = 0.0;
            nfc[2] = omegaCSq * sPlanePoleMagSq / sPlaneZeroMagSq;
...
//------------------------------------------------------------------------------
void hwAnalogFilterGen_ZP::CreateBandPassFilter()
{
    // coefficients are stored in descending powers of s
    ...
    else
    {
        ...
        numerCoef[0] = gainFactor;
        numerCoef[1] = 0.0;
        numerCoef[2] = 2.0 * bandCenterSq * gainFactor + sPlanePoleMagSq * bandWidth * bandWidth;
        numerCoef[3] = 0.0;
        numerCoef[4] = bandCenterSq * bandCenterSq * gainFactor;
        denomCoef[0] = 1.0;
        ...
        for (int i = 1; i < m_pFilterSpecs->Order()/2; ++i)
        {
            ...
            nfc[0] = gainFactor;
            nfc[1] = 0.0;
            nfc[2] = 2.0 * bandCenterSq * gainFactor + sPlanePoleMagSq * bandWidth * bandWidth;
            nfc[3] = 0.0;
            nfc[4] = bandCenterSq * bandCenterSq * gainFactor;
            dfc[0] = 1.0;
...
//------------------------------------------------------------------------------
void hwAnalogFilterGen_ZP::CreateBandStopFilter()
{
    // coefficients are stored in descending powers of s
    double bandWidth = m_pFilterSpecs->GetUpperCorner() - m_pFilterSpecs->GetLowerCorner();
    double bandCenterSq = m_pFilterSpecs->GetUpperCorner() * m_pFilterSpecs->GetLowerCorner();
    ...
    else
    {
        ...
        numerCoef[0] = sPlanePoleMagSq;
        numerCoef[1] = 0.0;
        numerCoef[2] = 2.0 * sPlanePoleMagSq * bandCenterSq + bandWidth * bandWidth * gainFactor;
        numerCoef[3] = 0.0;
        ...
        for (int i = 1; i < m_pFilterSpecs->Order()/2; ++i)
        {
            ...
            nfc[0] = sPlanePoleMagSq;
            nfc[1] = 0.0;
            nfc[2] = 2.0 * sPlanePoleMagSq * bandCenterSq + bandWidth * bandWidth * gainFactor;
            nfc[3] = 0.0;
