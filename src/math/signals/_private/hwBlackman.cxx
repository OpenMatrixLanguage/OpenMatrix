...
// Constructor
//------------------------------------------------------------------------------
hwBlackman::hwBlackman(bool periodic)
...
// Constructor
//------------------------------------------------------------------------------
hwBlackman::hwBlackman(int  num_points,
                       bool periodic)
...
// Destructor
//------------------------------------------------------------------------------
hwBlackman::~hwBlackman()
{
}
//------------------------------------------------------------------------------
// Returns status after computing window weights
//------------------------------------------------------------------------------
hwMathStatus hwBlackman::ComputeWeights(hwMatrix& weight)
{
    if (!weight.IsReal())
    {
    ...
    if (m_windowType == Periodic)
    {
        ...
        {
            weight(j) = 0.42 - 0.5 * cos((2*PI*j) / size)
                      + 0.08 * cos((4*PI*j) / size);
        }
    }
    else // (m_windowType == Symmetric)
    {
        ...
        else
        {
            for (int j = 0; j < size; ++j)
            {
                weight(j) = 0.42 - 0.5 * cos((2*PI*j) / (size - 1.0))
                          + 0.08 * cos((4*PI*j) / (size - 1.0));
            }
        }
    }
