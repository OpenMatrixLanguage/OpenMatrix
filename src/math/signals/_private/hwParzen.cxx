...
// Constructor
//------------------------------------------------------------------------------
hwParzen::hwParzen(bool periodic)
...
// Constructor
//------------------------------------------------------------------------------
hwParzen::hwParzen(int  num_points,
                   bool periodic)
...
//------------------------------------------------------------------------------
hwMathStatus hwParzen::ComputeWeights(hwMatrix& weight)
{
    // Note: the implementation is not the same as in HG
    if (!weight.IsReal())
    {
    ...
    {
        x = fabs((j - center) / half);

        if (x < 0.5)
        {
            weight(j) = 1.0 - 6.0 * x * x * (1.0 - x);
        }
        else
        {
            weight(j) = 2.0 * (1.0 - x) * (1.0 - x) * (1.0 - x);
        }
    }
