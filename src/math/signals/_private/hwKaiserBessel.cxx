...
// Constructor
//------------------------------------------------------------------------------
hwKaiserBessel::hwKaiserBessel(double beta,
                               bool   periodic)
...
// Constructor
//------------------------------------------------------------------------------
hwKaiserBessel::hwKaiserBessel(int    numPoints,
                               double beta,
                               bool   periodic)
...
// Destructor
//------------------------------------------------------------------------------
hwKaiserBessel::~hwKaiserBessel()
{
}
//------------------------------------------------------------------------------
// Returns status after computing window weights
//------------------------------------------------------------------------------
hwMathStatus hwKaiserBessel::ComputeWeights(hwMatrix& weight)
{
    if (!weight.IsReal())
    {
    ...
    double k3;
    double value;
    double denom = Bessel(m_beta);

    if (m_windowType == Periodic)
    {
        for (int j = 0; j < size; ++j)
        {
            value = ((double) (j + j - size)) / size;
            k3 = sqrt(1.0 - value * value);
            weight(j) = Bessel(m_beta * k3) / denom;
        }
    }
