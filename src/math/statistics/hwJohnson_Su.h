...
*/
#ifndef _Stats_hwJohnson_Su_h
#define _Stats_hwJohnson_Su_h

#include "hwDistribution.h"

class hwMersenneTwisterState;   // forward declaration
class hwNormal;                 // forward declaration

//------------------------------------------------------------------------------
//!
//! \class hwJohnson_Su
//! \brief Johynson Su distribution class
//!
//------------------------------------------------------------------------------
class hwJohnson_Su : public hwDistribution
{
public:
    //!
    //! Constructor
    ...
    //! \param pMTState
    //!
    hwJohnson_Su(double                  beta1,
                 double                  beta2,
                 hwMersenneTwisterState* pMTState = nullptr);
    //!
    //! Destructor
    //!
    virtual ~hwJohnson_Su();
...
private:
    double    m_delta;    //!< scale parameter
    double    m_gamma;    //!< location parameter
    double    m_mean;     //!< mean
    double    m_variance; //!< variance
    hwNormal* m_pNormal;  //!< pointer to standard normal distribution

    //!
    //! Compute Omega and omega
    //! \param beta1
    //! \param beta2
    //! \param lowerCaseOmega
    //! \param upperCaseOmega
    //!
    void ComputeOmegaValues(double  beta1,
                            double  beta2,
                            double& lowerCaseOmega,
                            double& upperCaseOmega);
};

#endif // _Stats_hwJohnson_Su_h
